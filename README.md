...
This is destructive by design and unconditional — asking for a wipe wipes; it's a dev system with no real data. It runs `./scripts/dev-services down -v` and `up`, `./scripts/db-migrate`, then `crm-admin bootstrap-platform-admin` (creates `owner@platform.test` via the migrator connection — the only step that uses it, alongside migrations the other sanctioned exception to "no direct database writes", D-021). Everything else — "Acme Realty" (Alice admin, Carol member) and "Best Realty" (Bob admin, Dave member) — is created by `scripts/seed_dev.py` entirely through the live HTTP API (create Organization, invite admin, accept, invite member, accept), the same sequence a real platform admin and org admin would drive by hand.

Seeding talks to the **running** dev API at `CRM_DEMO_API_URL` (default `http://127.0.0.1:3000`), so start `./scripts/dev-api` first — the script checks it's alive before wiping and waits for its lazy SQLx pool to reconnect to the fresh database afterwards. Existing sessions die with the wipe; just log in again. If the running process predates your current code, restart it first — an orphaned `dev-api` serving a stale binary would seed through old logic (see the note under Development scripts). `scripts/seed_dev.py` needs the `requests` package: `pip install -r scripts/requirements.txt`. `./scripts/demo-leads` (below) is unaffected.
...
Calling needs a LiveKit server with public UDP, so it runs on a separate public-IP Linux host (`livekit1.tarams.org`), not on the Mac and never through the tunnel (D-016 §5, AGENTS §7). Since 2026-09-09 the host behind `livekit1.tarams.org` is an EC2 `c6i.xlarge` in us-west-1 (security group `livekit-dev`; the same stack plus egress, writing recordings to the bucket `elysium-dev-recordings-337911129499` through the `livekit-egress-dev` instance role); the earlier OVH box is no longer referenced by DNS. Everything on the host is in `infra/telephony/`: `compose.yaml` (Caddy for TLS, Redis, LiveKit server with built-in TURN, LiveKit SIP — all host networking), templates rendered from the host's gitignored `.env` by `deploy.sh`, and `firewall.sh` (ufw: 22, 80/443 TCP, 7881 TCP, 50000–60000 UDP media, 3478 UDP + 5349 TCP TURN, 30000–40000 UDP TURN relay, 5060 SIP, 10000–20000 UDP SIP RTP). DNS is a plain, unproxied A record; Caddy obtains the Let's Encrypt certificate and LiveKit's TURN reuses it from Caddy's data volume.

Bring-up on a fresh Ubuntu 24.04: install `docker.io docker-compose-v2 gettext-base`, copy the directory to `/opt/crm-telephony`, fill `.env` from `.env.example` (the `LIVEKIT_API_KEY`/`SECRET` pair must match the Mac's `.env` — the one manual sync), run `./firewall.sh` then `./deploy.sh`. The stack also runs LiveKit Egress (recording) co-located with the server: set `EGRESS_S3_BUCKET`/`AWS_REGION` in the host `.env` and give the instance an IAM role that can write to that bucket (no S3 keys in any file — egress uses the instance role); leave `EGRESS_S3_BUCKET` empty to run without a default S3 target. **Recording is not wired into the product:** no application code starts an egress, and O-002 (consent) and O-012 (recording storage) remain open, so egress is dormant capability (D-055). Egress runs with host networking and `SYS_ADMIN`, and a failed upload leaves its file on the `egress_tmp` volume until removed by hand. Egress admits an audio-only room composite at 0.5 CPU units (1 with a custom layout) while host CPU stays under 80 %, so a 4 vCPU host records about five calls at once. Verify from the Mac: `curl https://livekit1.tarams.org/` answers 200, and a `RoomService/CreateRoom` Twirp call with a token signed by the key succeeds. The Telnyx SIP trunk is created once with `scripts/telephony-trunk` (reads `TELNYX_SIP_*` from `.env`); the API only ever sees the resulting `LIVEKIT_SIP_OUTBOUND_TRUNK_ID`. Host logs carry SIP URIs and numbers — treat them as PII (retention is the default Docker json-file rotation; tighten before production).
