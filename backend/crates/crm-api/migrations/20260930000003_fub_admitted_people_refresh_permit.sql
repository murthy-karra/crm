-- Close the item/baseline identity gaps found during the D-080 permit review.
ALTER TABLE migration_admitted_people_refresh_item ADD COLUMN baseline_version BIGINT NOT NULL DEFAULT 1 CHECK(baseline_version>0);
...
BEGIN
 ...
 JOIN migration_admitted_people_refresh r ON r.id=i.refresh_id AND r.organization_id=i.organization_id AND r.confirmed_refresh_plan_id=i.plan_id
 ...
 JOIN migration_import_identity mi ON mi.organization_id=i.organization_id AND mi.family='people' AND mi.source_id=i.source_id AND mi.target_id=i.person_id AND mi.admission_id=r.admission_id AND mi.admission_item_id=i.admission_item_id AND mi.admission_result_id=ar.id
 JOIN migration_admitted_people_refresh_baseline b ON b.organization_id=i.organization_id AND b.admission_id=r.admission_id AND b.source_id=i.source_id AND b.person_id=i.person_id AND b.admission_result_id=ar.id AND b.version=i.baseline_version
