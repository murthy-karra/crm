...
-- This root is intentionally linked to the immutable admission result set, not
-- to a synthetic 010c activity child.  The parent tuple remains the workspace
-- binding; `admission_*` and `source_report_*` identify the admitted cohort and
-- the separately qualified newer capture.
CREATE TABLE migration_admitted_activity_import (
 ...
 admission_id UUID NOT NULL, admission_plan_id UUID NOT NULL, source_report_id UUID NOT NULL,
 ...
 FOREIGN KEY(predecessor_import_id,organization_id) REFERENCES migration_admitted_activity_import(id,organization_id),
...
CREATE TABLE migration_admitted_activity_plan (
 ...
 FOREIGN KEY(import_id,snapshot_id,organization_id) REFERENCES migration_admitted_activity_import(id,snapshot_id,organization_id),
 FOREIGN KEY(parent_plan_id,import_id,organization_id) REFERENCES migration_admitted_activity_plan(id,import_id,organization_id),
 FOREIGN KEY(inherit_plan_id,import_id,organization_id) REFERENCES migration_admitted_activity_plan(id,import_id,organization_id)
);
ALTER TABLE migration_admitted_activity_import ADD FOREIGN KEY(latest_plan_id,id,organization_id) REFERENCES migration_admitted_activity_plan(id,import_id,organization_id);
ALTER TABLE migration_admitted_activity_import ADD FOREIGN KEY(confirmed_plan_id,id,organization_id) REFERENCES migration_admitted_activity_plan(id,import_id,organization_id);
CREATE TABLE migration_admitted_activity_choice (
 ...
 FOREIGN KEY(plan_id,import_id,organization_id) REFERENCES migration_admitted_activity_plan(id,import_id,organization_id)
);
CREATE TABLE migration_admitted_activity_source (
...
CREATE INDEX migration_admitted_activity_source_page ON migration_admitted_activity_source(plan_id,organization_id,id);
CREATE INDEX migration_admitted_activity_source_identity ON migration_admitted_activity_source(plan_id,organization_id,family,source_id,representation,id);
CREATE INDEX migration_admitted_activity_source_negative ON migration_admitted_activity_source(plan_id,organization_id,semantic_hmac) WHERE negative;
CREATE TABLE migration_admitted_activity_mapping (
 id UUID PRIMARY KEY, plan_id UUID NOT NULL, import_id UUID NOT NULL, organization_id UUID NOT NULL,
 kind TEXT NOT NULL CHECK(kind IN ('note_author','task_creator','task_assignee','task_kind')),
...
CREATE INDEX migration_admitted_activity_mapping_page ON migration_admitted_activity_mapping(plan_id,organization_id,id);
CREATE TABLE migration_admitted_activity_manifest (
 id UUID PRIMARY KEY, plan_id UUID NOT NULL, import_id UUID NOT NULL, organization_id UUID NOT NULL,
 kind TEXT NOT NULL CHECK(kind IN ('note','task')), source_id TEXT CHECK(length(source_id)<=128), source_row_id UUID NOT NULL,
 source_person_id TEXT CHECK(length(source_person_id)<=128), admission_result_id UUID, person_id UUID, target_id UUID,
 ...
 FOREIGN KEY(plan_id,import_id,organization_id) REFERENCES migration_admitted_activity_plan(id,import_id,organization_id),
 FOREIGN KEY(source_row_id,plan_id,import_id,organization_id) REFERENCES migration_admitted_activity_source(id,plan_id,import_id,organization_id),
...
CREATE UNIQUE INDEX migration_admitted_activity_manifest_identity ON migration_admitted_activity_manifest(plan_id,organization_id,kind,source_id) WHERE source_id IS NOT NULL;
CREATE INDEX migration_admitted_activity_manifest_page ON migration_admitted_activity_manifest(plan_id,organization_id,id);
CREATE INDEX migration_admitted_activity_manifest_filter ON migration_admitted_activity_manifest(plan_id,organization_id,kind,disposition,id);
CREATE INDEX migration_admitted_activity_manifest_person ON migration_admitted_activity_manifest(organization_id,person_id,id);
CREATE TABLE migration_admitted_activity_manifest_issue (
 ...
 FOREIGN KEY(manifest_id,plan_id,import_id,organization_id) REFERENCES migration_admitted_activity_manifest(id,plan_id,import_id,organization_id)
);
CREATE INDEX migration_admitted_activity_manifest_issue_page ON migration_admitted_activity_manifest_issue(plan_id,organization_id,code,manifest_id);
-- `migration_activity_identity` remains the one global registry.  Old rows
-- retain their original tuple; admitted rows carry only this tuple.
ALTER TABLE migration_activity_identity
 ALTER COLUMN import_id DROP NOT NULL,
 ALTER COLUMN plan_id DROP NOT NULL,
 ALTER COLUMN manifest_id DROP NOT NULL,
 ADD COLUMN admitted_import_id UUID,
 ADD COLUMN admitted_plan_id UUID,
 ADD COLUMN admitted_manifest_id UUID,
 ADD CONSTRAINT migration_activity_identity_admitted_owner_fk
   FOREIGN KEY(admitted_manifest_id,admitted_plan_id,admitted_import_id,organization_id)
   REFERENCES migration_admitted_activity_manifest(id,plan_id,import_id,organization_id),
 ADD CONSTRAINT migration_activity_identity_exclusive_owner CHECK (
   ...
   OR
   ((import_id IS NULL AND plan_id IS NULL AND manifest_id IS NULL)
    AND admitted_import_id IS NOT NULL AND admitted_plan_id IS NOT NULL AND admitted_manifest_id IS NOT NULL)
 );
CREATE TABLE migration_admitted_activity_result (
...
CREATE INDEX migration_admitted_activity_result_page ON migration_admitted_activity_result(plan_id,organization_id,id);
CREATE INDEX migration_admitted_activity_result_filter ON migration_admitted_activity_result(plan_id,organization_id,kind,disposition,id);
CREATE INDEX migration_admitted_activity_result_person ON migration_admitted_activity_result(organization_id,person_id,kind,target_id,id);
-- Immutable execution issues are distinct from the frozen plan's issues.
CREATE TABLE migration_admitted_activity_result_issue (
 ...
 FOREIGN KEY(result_id,plan_id,import_id,organization_id) REFERENCES migration_admitted_activity_result(id,plan_id,import_id,organization_id)
);
CREATE INDEX migration_admitted_activity_result_issue_page ON migration_admitted_activity_result_issue(plan_id,organization_id,code,result_id);
CREATE TABLE migration_admitted_activity_receipt (
 ...
 FOREIGN KEY(import_id,organization_id) REFERENCES migration_admitted_activity_import(id,organization_id)
);
CREATE TABLE migration_admitted_activity_reservation (
...
CREATE INDEX migration_admitted_activity_reservation_owner ON migration_admitted_activity_reservation(import_id,organization_id,purpose);
CREATE TABLE migration_admitted_activity_issue (
...
GRANT SELECT,INSERT,UPDATE ON migration_admitted_activity_import,migration_admitted_activity_plan,migration_admitted_activity_mapping,migration_admitted_activity_issue TO crm_app;
GRANT SELECT,INSERT ON migration_admitted_activity_choice,migration_admitted_activity_source,migration_admitted_activity_manifest,migration_admitted_activity_manifest_issue,migration_admitted_activity_result,migration_admitted_activity_result_issue,migration_admitted_activity_receipt TO crm_app;
GRANT SELECT,INSERT,DELETE ON migration_admitted_activity_reservation TO crm_app;
GRANT UPDATE(byte_count) ON migration_admitted_activity_reservation TO crm_app;
CREATE TRIGGER migration_admitted_activity_receipt_append_only BEFORE UPDATE OR DELETE ON migration_admitted_activity_receipt FOR EACH ROW EXECUTE FUNCTION reject_mutation();
...
CREATE FUNCTION crm_admitted_activity_retained_size(table_name TEXT,v JSONB) RETURNS BIGINT LANGUAGE plpgsql IMMUTABLE AS $$
...
BEGIN
 ...
 RETURN total;
END $$;
CREATE FUNCTION crm_admitted_activity_measure_row() RETURNS TRIGGER LANGUAGE plpgsql AS $$
...
DO $$ DECLARE name TEXT; BEGIN
 FOREACH name IN ARRAY ARRAY['migration_admitted_activity_import','migration_admitted_activity_plan','migration_admitted_activity_choice','migration_admitted_activity_source','migration_admitted_activity_mapping','migration_admitted_activity_manifest','migration_admitted_activity_manifest_issue','migration_admitted_activity_result','migration_admitted_activity_result_issue','migration_admitted_activity_receipt','migration_admitted_activity_issue'] LOOP
  EXECUTE format('CREATE TRIGGER %I AFTER INSERT OR UPDATE OR DELETE ON %I FOR EACH ROW EXECUTE FUNCTION crm_admitted_activity_measure_row()',name||'_measure',name);
 END LOOP;
END $$;
REVOKE ALL ON FUNCTION crm_admitted_activity_retained_size(TEXT,JSONB),crm_admitted_activity_measure_row() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION crm_admitted_activity_retained_size(TEXT,JSONB),crm_admitted_activity_measure_row() TO crm_app;

-- The existing identity trigger used the original `import_id` as its owner.
-- Resolve either exclusive tuple so new registry rows are charged once to the
-- admitted root while old rows continue charging their original child.
CREATE OR REPLACE FUNCTION crm_activity_measure_row() RETURNS TRIGGER LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 IF TG_TABLE_NAME='migration_activity_identity' AND row_value->>'admitted_import_id' IS NOT NULL THEN
...
CREATE OR REPLACE FUNCTION crm_activity_complete_read(org UUID) RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
...
CREATE FUNCTION crm_admitted_activity_insert_allowed(org UUID,table_name TEXT,row_value JSONB) RETURNS BOOLEAN LANGUAGE plpgsql AS $$
