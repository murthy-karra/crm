...
pub async fn run(config: Config) -> Result<(), BoxError> {
    ...
    let _people_refresh_worker = state.db.as_ref().map(|pool| {
        let pool = pool.clone();
        let state = state.clone();
        tokio::spawn(async move {
            let mut tick = tokio::time::interval(std::time::Duration::from_secs(1));
    ...
    let _admitted_people_refresh_worker = state.db.as_ref().map(|pool| {
        let pool = pool.clone();
        let state = state.clone();
        tokio::spawn(async move {
            let mut tick = tokio::time::interval(std::time::Duration::from_secs(1));
            ...
            loop {
                ...
                for _ in 0..32 {
                    ...
                    .await
                    {
                        ...
                        Ok(false) => break,
                        Err(error) => {
                            tracing::warn!(outcome=%error, "admitted people refresh sweep failed");
                            break;
                        }
                    }
                }
    ...
    let _admitted_metadata_worker = state.db.as_ref().map(|pool| {
        ...
        tokio::spawn(async move {
            ...
            loop {
                tick.tick().await;
                for _ in 0..32 {
                    match domain::migration::admitted_metadata::with_policy(
                        &policy,
                        domain::migration::admitted_metadata_worker::run_once(&pool, &key),
                    )
                    .await
                    {
                        Ok(true) => tokio::task::yield_now().await,
