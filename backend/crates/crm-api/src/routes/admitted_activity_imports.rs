...
use crate::{
    ...
    domain::{envelope::CommandContext, migration::admitted_activity as i},
    error::ApiError,
    state::AppState,
};
use axum::{
...
pub fn router() -> Router<AppState> {
    Router::new()
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/records/{record}/observations",
            get(observations),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports",
            get(list).post(prepare),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}",
            get(detail),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/plans",
            post(replan),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/confirm",
            post(confirm),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/retry",
            post(retry),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/cancel",
            post(cancel),
        )
        .route(
        ...
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/mappings",
            get(mappings),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/results",
            get(results),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/targets",
            get(targets),
        )
        .route(
            "/api/migrations/fub/admitted-activity-imports/{id}/{endpoint}/{row}/fields/{field}",
            get(field),
        )
...
async fn ready(s: &AppState) -> bool {
    s.current_import_release()
        .await
        .is_some_and(|v| v.admitted_activity_ready())
}
async fn list(
...
async fn prepare(
    State(s): State<AppState>,
    a: OrgAdminContext,
    b: Result<Json<i::PrepareAdmittedActivityImport>, JsonRejection>,
) -> Result<Response, ApiError> {
    ...
    release
        .require_admitted_activity(&mut *pool.acquire().await.map_err(|_| ApiError::Unavailable)?)
        .await
...
async fn replan(
    ...
    a: OrgAdminContext,
    p: Result<Path<Uuid>, PathRejection>,
    b: Result<Json<i::PlanAdmittedActivityImport>, JsonRejection>,
) -> Result<Response, ApiError> {
    Ok(response(
        StatusCode::ACCEPTED,
        i::replan(
...
async fn confirm(
    ...
    a: OrgAdminContext,
    p: Result<Path<Uuid>, PathRejection>,
    b: Result<Json<i::ConfirmAdmittedActivityImport>, JsonRejection>,
) -> Result<Response, ApiError> {
    let id = path(p)?;
    let cmd = body(b)?;
    let pool = s.db.as_ref().ok_or(ApiError::Unavailable)?;
...
async fn retry(
    ...
    a: OrgAdminContext,
    p: Result<Path<Uuid>, PathRejection>,
    b: Result<Json<i::AdmittedActivityAction>, JsonRejection>,
) -> Result<Response, ApiError> {
    Ok(response(
        StatusCode::ACCEPTED,
        i::action(
