...
use crate::{
    ...
    domain::{
        ...
        migration::{admitted_people_refresh as h, MigrationError},
    },
    error::ApiError,
    state::AppState,
};
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/items/{item}",
            get(item),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/items/{item}/contacts",
            get(contacts),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/plans",
            post(repreview),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/confirm",
            post(confirm),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/retry",
            post(retry),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/cancel",
            post(cancel),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/results",
            get(results),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/items/{item}/fields/{side}/{field}",
            get(field),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/mapping-repairs",
            post(prepare_repair),
        )
        .route(
            "/api/migrations/fub/admitted-people-refreshes/{id}/repair-mappings",
            get(repair_mappings)
...
async fn prepare(
    ...
    a: OrgAdminContext,
    b: Result<Json<h::PrepareAdmittedPeopleRefresh>, JsonRejection>,
...
async fn repreview(
    ...
    a: OrgAdminContext,
    p: Result<Path<Uuid>, PathRejection>,
    b: Result<Json<h::RepreviewAdmittedPeopleRefresh>, JsonRejection>,
) -> Result<Response, ApiError> {
    Ok(response(
        StatusCode::ACCEPTED,
        h::repreview(
...
async fn confirm(
    ...
    a: OrgAdminContext,
    p: Result<Path<Uuid>, PathRejection>,
    b: Result<Json<h::ConfirmAdmittedPeopleRefresh>, JsonRejection>,
) -> Result<Response, ApiError> {
    let release = s.current_import_release().await;
    Ok(response(
        StatusCode::ACCEPTED,
...
async fn retry(
    ...
    a: OrgAdminContext,
    p: Result<Path<Uuid>, PathRejection>,
    b: Result<Json<h::LifecycleAdmittedPeopleRefresh>, JsonRejection>,
) -> Result<Response, ApiError> {
    let release = s.current_import_release().await;
    Ok(response(
        StatusCode::ACCEPTED,
