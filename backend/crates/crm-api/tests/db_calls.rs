//! DB-backed tests for the Slice 006 call routes (start/dial/hangup), the
//! LiveKit webhook, and the sweep (`run_once`, driven directly with
//! backdated rows), over the `ScriptedProvider` (docs/specs/SLICE_006.md
//! §13 item 2) and the `call_completed` history kind. Split from a single
//! db_calls.rs (item 3 of the LATER batch,
//! docs/tasks/LATER_BATCH_2026-09-08.md, no test bodies changed):
//! call-outcome corrections moved to db_calls_corrections.rs, and the
//! Today "low"/outcome-needed items moved to db_calls_outcome_today.rs.
//! The shared telephony fixture harness (`Fixture`, `fixture()`,
//! `dial`/`hangup`/`start`, etc.) moved to tests/common/calls.rs, used by
//! all three files. Run only via ./scripts/check-db.
...
use crm_api::telephony::{DialOutcome, ProviderError, RecordedCall, SipFailure, Telephony};
