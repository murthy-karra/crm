...
async fn cross_organization_people_stages_and_unresolved_are_isolated(migrator_pool: PgPool) {
    ...
    let a_resp = crate::common::post_inquiry(
        &router,
        &alice_cookie,
        "zillow",
    ...
    .await;
    let a_person_id: Uuid = crate::common::body_json(a_resp).await["person_id"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();

    let b_resp = crate::common::post_inquiry(
        &router,
        &bob_cookie,
        "zillow",
        json!({ "email": "shared@example.com" }),
    ...
    .await;
    let b_person_id: Uuid = crate::common::body_json(b_resp).await["person_id"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    ...
    let get_resp =
        crate::common::get_with_cookie(&router, &format!("/api/people/{a_person_id}"), &bob_cookie)
            .await;
    assert_eq!(get_resp.status(), StatusCode::NOT_FOUND);

    let assign_resp = crate::common::post_json_with_cookie(
        &router,
    ...
    let (stage_fact_before,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM stage_changed WHERE person_id = $1")
            ...
            .await
            .unwrap();
    let cross_stage_resp = crate::common::post_json_with_cookie(
        &router,
        &format!("/api/people/{a_person_id}/stage"),
        &alice_cookie,
    ...
    assert_eq!(
        crate::common::body_json(cross_stage_resp).await["error"],
        "invalid_stage"
    );
    ...
    let (assign_fact_before,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM assignment_changed WHERE person_id = $1")
            ...
            .await
            .unwrap();
    let cross_assign_resp = crate::common::post_json_with_cookie(
        &router,
        &format!("/api/people/{a_person_id}/assignment"),
        &alice_cookie,
        json!({ "assigned_user_id": bob_id }),
    ...
    assert_eq!(
        crate::common::body_json(cross_assign_resp).await["error"],
        "invalid_assignee"
    );
    ...
    let people_body = crate::common::body_json(
        crate::common::get_with_cookie(&router, "/api/people", &alice_cookie).await,
    )
    .await;
    ...
    let stages_body = crate::common::body_json(
        crate::common::get_with_cookie(&router, "/api/stages", &alice_cookie).await,
    )
    .await;
    ...
    .await;
    let alice_unresolved = crate::common::body_json(
        crate::common::get_with_cookie(&router, "/api/intake/unresolved", &alice_cookie).await,
    )
    .await;
    let bob_unresolved = crate::common::body_json(
    ...
    let probe = router
        ...
        .await
        .unwrap();
    let probe_body = crate::common::body_json(probe).await;
...
async fn assign_and_stage_commands_are_fact_precise_and_history_orders_stably(
    ...
    let resp1 = crate::common::post_json_with_cookie(
        &router,
    ...
    .await;
    assert_eq!(crate::common::body_json(resp1).await["changed"], true);
    ...
    let (assign_count_before,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM assignment_changed WHERE person_id = $1")
            ...
            .await
            .unwrap();
    let resp2 = crate::common::post_json_with_cookie(
        &router,
        &format!("/api/people/{person_id}/assignment"),
        &cookie,
        json!({ "assigned_user_id": carol_id }),
    ...
    .await;
    assert_eq!(crate::common::body_json(resp2).await["changed"], false);
    ...
    let stages_body = crate::common::body_json(
        crate::common::get_with_cookie(&router, "/api/stages", &cookie).await,
    )
    .await;
    let second_stage_id: Uuid = stages_body["stages"][1]["id"]
        ...
        .parse()
        .unwrap();
    let stage_resp1 = crate::common::post_json_with_cookie(
        &router,
    ...
    .await;
    assert_eq!(crate::common::body_json(stage_resp1).await["changed"], true);
    ...
    let (stage_count_before,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM stage_changed WHERE person_id = $1")
            ...
            .await
            .unwrap();
    let stage_resp2 = crate::common::post_json_with_cookie(
        &router,
        &format!("/api/people/{person_id}/stage"),
        &cookie,
        json!({ "stage_id": second_stage_id }),
    ...
    .await;
    assert_eq!(
        crate::common::body_json(stage_resp2).await["changed"],
        false
    );
    ...
    let detail_body = crate::common::body_json(
...
/// flow (`crate::common::create_org_with_admin_and_member_via_api` — the same
...
async fn organization_creation_seeds_nine_ordered_stages_and_two_members(migrator_pool: PgPool) {
    ...
    .await;
    let router = crate::common::build_router(&migrator_pool).await;
    let platform_cookie = crate::common::login_cookie(&router, "owner@platform.test", PW).await;
    ...
    .await;
    let best_id = crate::common::create_org_with_admin_and_member_via_api(
        &router,
        &platform_cookie,
        PW,
        "Best Realty",
    ...
    let dup = crate::common::post_json_with_cookie(
        &router,
