...
async fn audit(f: &Fixture, run: Uuid) -> Ledger {
    // D-084 retained mapping evidence also exists on ordinary refresh plans.
    // Keep this physical audit independent of runtime charging, including repair
    // rows and transferred mapping heads, rather than auditing ciphertext alone.
    let actual:i64=sqlx::query_scalar("SELECT COALESCE((SELECT sum(octet_length(inputs_nonce)+octet_length(inputs_ciphertext)+COALESCE(octet_length(digest),0)+COALESCE(octet_length(repair_choices_digest),0)) FROM migration_people_refresh_plan WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(proposed_nonce)+octet_length(proposed_ciphertext)+octet_length(baseline_nonce)+octet_length(baseline_ciphertext)+octet_length(current_nonce)+octet_length(current_ciphertext)+octet_length(instructions_nonce)+octet_length(instructions_ciphertext)+COALESCE(octet_length(mapping_evidence_nonce),0)+COALESCE(octet_length(mapping_evidence_ciphertext),0)+COALESCE(octet_length(native_fingerprint),0)+COALESCE(octet_length(repair_stage_source_hmac),0)+COALESCE(octet_length(repair_assignee_source_hmac),0)) FROM migration_people_refresh_item WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(value_nonce)+octet_length(value_ciphertext)) FROM migration_people_refresh_contact WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(before_nonce)+octet_length(before_ciphertext)+octet_length(after_nonce)+octet_length(after_ciphertext)) FROM migration_people_refresh_result WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(nonce)+octet_length(ciphertext)) FROM migration_people_refresh_receipt WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(projection_nonce)+octet_length(projection_ciphertext)) FROM migration_people_refresh_baseline WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(nonce)+octet_length(ciphertext)+octet_length(source_key_hmac)) FROM migration_people_refresh_repair_key WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(nonce)+octet_length(ciphertext)+octet_length(source_key_hmac)) FROM migration_people_refresh_repair_choice WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(source_id)+COALESCE(octet_length(stage_source_hmac),0)+COALESCE(octet_length(assignee_source_hmac),0)) FROM migration_people_refresh_repair_candidate WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(source_id)+octet_length(source_key_hmac)) FROM migration_people_refresh_mapping_binding WHERE refresh_id=$1 AND organization_id=$2),0) + COALESCE((SELECT sum(octet_length(h.source_key_hmac)) FROM migration_people_refresh_mapping_head h JOIN migration_people_refresh_mapping_binding b ON b.id=h.binding_id AND b.organization_id=h.organization_id WHERE b.refresh_id=$1 AND h.organization_id=$2),0) + COALESCE((SELECT octet_length(repair_choices_digest) FROM migration_people_refresh WHERE id=$1 AND organization_id=$2),0)").bind(run).bind(f.org).fetch_one(&f.pool).await.unwrap();
    ...
    assert_eq!(
        state.run, actual,
        "run ledger must match exact retained variable-width evidence"
    );
