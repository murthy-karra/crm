//! DB-backed saved-list coverage (Slice 011b): schema/grants, create/
//! replay/edit/delete, retry identity, visibility/permission and quota
//! rules, concurrent-create/membership races, saved-count parity, stored
//! JSONB robustness, the HTTP contract, and span redaction. These tests
//! deliberately use the production command/read path for authority,
//! revisions, retry identity, and count semantics; direct SQL is limited
//! to fixture setup and adversarial stored JSONB rows that normal
//! application writes cannot create. Split from a single db_saved_lists.rs
//! (item 3 of the LATER batch, docs/tasks/LATER_BATCH_2026-09-08.md, no
//! test bodies changed): the Slice 011b-sort feature moved to
//! db_saved_lists_sort.rs, and the tag interplay (Slice 011e e2) moved to
//! db_saved_lists_tags.rs. The shared fixture helpers (`create_list`,
//! `empty_filter`, `command_context`, `auth_context`, `delete_json`,
//! `assert_status_body`) moved to tests/common/saved_lists.rs, used by
//! this file and db_saved_lists_sort.rs. Run only via ./scripts/check-db.
