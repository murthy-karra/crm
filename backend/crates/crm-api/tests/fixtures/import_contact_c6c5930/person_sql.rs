...
/// Binds the full 72-parameter matrix, matching the `c6c5930` statements
...
/// NULL-guarded 71-parameter matrix (no `reference_now` — this statement
/// always uses `now()`), capped at 501 in a subquery. The fixture keeps the
/// verified `c6c5930` text in a fixed `include_str!` file and uses runtime
