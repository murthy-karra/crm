//! Baseline `6bad52a` adapters used only by Slice 019b's performance test.
...
fn no_custom(params: &PersonFilterParams) -> Result<(), sqlx::Error> {
    if (0..5).any(|index| params.custom_slot(index).is_some()) {
        return Err(sqlx::Error::Decode(
            "frozen 6bad52a statement received custom slot".into(),
        ));
    }
    Ok(())
}
