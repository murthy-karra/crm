...
//! Copied byte-for-byte from `6bad52a`; only Rust import paths and the `.sql`
//! file paths were adjusted. Fixed `include_str!` statements plus runtime
//! SQLx bindings leave this test-only fixture outside production macro
//! metadata. `PersonStateRow`/`CallMembershipRow`/`CallOnlyRow` and their
