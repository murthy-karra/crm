...
const ADMITTED_ACTIVITY_SCHEMA: &str = "SELECT (SELECT bool_and(to_regclass('public.'||name) IS NOT NULL) FROM (VALUES ('migration_admitted_activity_import'),('migration_admitted_activity_plan'),('migration_admitted_activity_choice'),('migration_admitted_activity_source'),('migration_admitted_activity_mapping'),('migration_admitted_activity_manifest'),('migration_admitted_activity_result'),('migration_admitted_activity_receipt'),('migration_admitted_activity_reservation')) required(name)) AND to_regprocedure('public.crm_admitted_activity_insert_allowed(uuid,text,jsonb)') IS NOT NULL AND EXISTS(SELECT 1 FROM pg_constraint WHERE conrelid=to_regclass('public.migration_activity_identity') AND conname='migration_activity_identity_exclusive_owner' AND convalidated) AND (SELECT count(*)=3 FROM pg_attribute WHERE attrelid=to_regclass('public.migration_activity_identity') AND attname IN ('admitted_import_id','admitted_plan_id','admitted_manifest_id') AND atttypid='uuid'::regtype AND NOT attisdropped)";
