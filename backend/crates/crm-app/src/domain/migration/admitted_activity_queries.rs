...
use super::{
    admitted_activity::{self, ActivityPage, FieldQuery},
    admitted_activity_model::{self as m, Manifest, Mapping, ResultData},
    admitted_activity_store as s, metadata_model as display,
...
fn purpose(
    ...
    Ok(format!("activity-{endpoint}:{}",serde_json::to_string(&json!({"plan":plan,"revision":revision.to_string(),"kind":q.kind,"disposition":q.disposition,"issue":q.issue,"parent":q.admission_id,"limit":q.limit()?})).map_err(|_|MigrationError::Crypto)?))
}
...
fn validate(q: &ActivityPage, endpoint: &str) -> Result<i64, MigrationError> {
    ...
    if q.kind.as_deref().is_some_and(|v| !kinds.contains(&v))
        ...
        || endpoint != "list" && q.admission_id.is_some()
    {
...
pub async fn list(
    ...
    s::with_policy(policy,async{let mut tx=s::begin(pool,ctx,false).await?;let limit=validate(&q,"list")?;if let Some(admission)=q.admission_id{if !sqlx::query_scalar::<_,bool>("SELECT EXISTS(SELECT 1 FROM migration_people_admission WHERE id=$1 AND organization_id=$2)").bind(admission).bind(ctx.organization_id.0).fetch_one(&mut *tx).await?{return Err(MigrationError::NotFound)}}let purpose=purpose("list",Uuid::nil(),0,&q)?;let after=cursor(key,ctx.organization_id,Uuid::nil(),&purpose,q.cursor.as_deref())?.unwrap_or(Uuid::nil());let rows=sqlx::query_scalar::<_,Uuid>("SELECT id FROM migration_admitted_activity_import WHERE organization_id=$1 AND id>$2 AND ($3::uuid IS NULL OR admission_id=$3) ORDER BY id LIMIT $4").bind(ctx.organization_id.0).bind(after).bind(q.admission_id).bind(limit+1).fetch_all(&mut *tx).await?;let mut items=vec![];let mut used=4096;let mut last=None;for id in rows.iter().take(limit as usize){let r=s::run(&mut tx,ctx.organization_id,*id).await?;admitted_activity::validate_binding(&mut tx,&r).await?;let v=admitted_activity::view(&mut tx,key,ctx.organization_id,&r).await?;if !push(&mut items,&mut used,v)?{break}last=Some(*id)}finish(key,ctx.organization_id,Uuid::nil(),&purpose,items,rows.len(),last,"imports")}).await
}
...
async fn page(
    ...
    admitted_activity::validate_binding(&mut tx, &r).await?;
    ...
    let sql = match (endpoint, q.issue.is_some()) {
        ("records", true) => "SELECT a.id FROM migration_admitted_activity_manifest_issue x JOIN migration_admitted_activity_manifest a ON a.id=x.manifest_id AND a.plan_id=x.plan_id AND a.import_id=x.import_id AND a.organization_id=x.organization_id WHERE x.plan_id=$1 AND x.organization_id=$2 AND x.manifest_id>$3 AND ($4::text IS NULL OR a.kind=$4) AND ($5::text IS NULL OR a.disposition=$5) AND x.code=$6 ORDER BY x.manifest_id LIMIT $7",
        ("results", true) => "SELECT a.id FROM migration_admitted_activity_result_issue x JOIN migration_admitted_activity_result a ON a.id=x.result_id AND a.plan_id=x.plan_id AND a.import_id=x.import_id AND a.organization_id=x.organization_id WHERE x.plan_id=$1 AND x.organization_id=$2 AND x.result_id>$3 AND ($4::text IS NULL OR a.kind=$4) AND ($5::text IS NULL OR a.disposition=$5) AND x.code=$6 ORDER BY x.result_id LIMIT $7",
        ("records", false) => "SELECT a.id FROM migration_admitted_activity_manifest a WHERE a.plan_id=$1 AND a.organization_id=$2 AND a.id>$3 AND ($4::text IS NULL OR a.kind=$4) AND ($5::text IS NULL OR a.disposition=$5) AND $6::text IS NULL ORDER BY a.id LIMIT $7",
        ("results", false) => "SELECT a.id FROM migration_admitted_activity_result a WHERE a.plan_id=$1 AND a.organization_id=$2 AND a.id>$3 AND ($4::text IS NULL OR a.kind=$4) AND ($5::text IS NULL OR a.disposition=$5) AND $6::text IS NULL ORDER BY a.id LIMIT $7",
        _ => "SELECT id FROM migration_admitted_activity_mapping WHERE plan_id=$1 AND organization_id=$2 AND id>$3 AND ($4::text IS NULL OR kind=$4) AND $5::text IS NULL AND $6::text IS NULL ORDER BY id LIMIT $7",
    };
    ...
    for rowid in rows.iter().take(limit as usize) {
        let sql=match endpoint{"records"=>"SELECT * FROM migration_admitted_activity_manifest WHERE id=$1 AND plan_id=$2 AND organization_id=$3","results"=>"SELECT * FROM migration_admitted_activity_result WHERE id=$1 AND plan_id=$2 AND organization_id=$3",_=>"SELECT * FROM migration_admitted_activity_mapping WHERE id=$1 AND plan_id=$2 AND organization_id=$3"};
        let row = sqlx::query(sql)
            ...
            .fetch_one(&mut *tx)
            .await?;
        let field_url=format!("/api/migrations/fub/admitted-activity-imports/{id}/{endpoint}/{rowid}/fields/all?plan_id={plan}");
...
pub async fn targets(
    ...
    let plan = q.plan_id.unwrap_or(r.get("latest_plan_id"));
    let p = s::plan(&mut tx, ctx.organization_id, id, plan).await?;
    let d = admitted_activity::decode_destination(key, &r, &p)?;
...
pub async fn field(
    ...
    let plan = q.plan_id.unwrap_or(r.get("latest_plan_id"));
    let p = s::plan(&mut tx, ctx.organization_id, id, plan).await?;
    let sql=match endpoint{"records"=>"SELECT * FROM migration_admitted_activity_manifest WHERE id=$1 AND plan_id=$2 AND import_id=$3 AND organization_id=$4","mappings"=>"SELECT * FROM migration_admitted_activity_mapping WHERE id=$1 AND plan_id=$2 AND import_id=$3 AND organization_id=$4","results"=>"SELECT * FROM migration_admitted_activity_result WHERE id=$1 AND plan_id=$2 AND import_id=$3 AND organization_id=$4","observations"=>"SELECT * FROM migration_admitted_activity_source WHERE id=$1 AND plan_id=$2 AND import_id=$3 AND organization_id=$4",_=>return Err(MigrationError::NotFound)};
    let row = sqlx::query(sql)
        .bind(rowid)
        .bind(plan)
        .bind(id)
...
pub async fn observations(
    ...
    let plan = q.plan_id.unwrap_or(r.get("latest_plan_id"));
    let p = s::plan(&mut tx, ctx.organization_id, id, plan).await?;
    let manifest=sqlx::query("SELECT source_id,kind,source_row_id FROM migration_admitted_activity_manifest WHERE id=$1 AND plan_id=$2 AND import_id=$3 AND organization_id=$4").bind(record).bind(plan).bind(id).bind(ctx.organization_id.0).fetch_optional(&mut *tx).await?.ok_or(MigrationError::NotFound)?;
