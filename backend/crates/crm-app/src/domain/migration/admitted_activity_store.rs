...
pub(crate) async fn run(
    ...
    sqlx::query(
        "SELECT * FROM migration_admitted_activity_import WHERE id=$1 AND organization_id=$2 FOR UPDATE",
    )
    .bind(id)
    .bind(org.0)
    .fetch_optional(conn)
...
pub(crate) async fn plan(
    ...
    sqlx::query("SELECT * FROM migration_admitted_activity_plan WHERE id=$1 AND import_id=$2 AND organization_id=$3 FOR UPDATE").bind(id).bind(child).bind(org.0).fetch_optional(conn).await?.ok_or(MigrationError::NotFound)
}
#[allow(clippy::too_many_arguments)] // Keep tenant, immutable evidence and lease/receipt scopes explicit.
pub(crate) async fn reserve(
    ...
    sqlx::query("UPDATE migration_admitted_activity_import SET reserved_bytes=reserved_bytes+$3 WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).bind(amount).execute(conn).await?;
    Ok(true)
}
pub(crate) async fn settle(
    ...
    let actual:i64=sqlx::query_scalar("SELECT measured_bytes-retained_bytes FROM migration_admitted_activity_import WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).fetch_one(&mut *conn).await?;
    let r=sqlx::query("DELETE FROM migration_admitted_activity_reservation WHERE token=$1 AND import_id=$2 AND organization_id=$3 RETURNING byte_count,snapshot_id").bind(token).bind(child).bind(org.0).fetch_optional(&mut *conn).await?.ok_or(MigrationError::ImportConflict)?;
    ...
    sqlx::query("UPDATE migration_admitted_activity_import SET reserved_bytes=reserved_bytes-$3,retained_bytes=retained_bytes+$4 WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).bind(reserved).bind(actual).execute(conn).await?;
    Ok(())
}
pub(crate) async fn release(
    ...
    let ids=sqlx::query_scalar::<_,Uuid>("SELECT token FROM migration_admitted_activity_reservation WHERE import_id=$1 AND organization_id=$2 AND purpose=$3 ORDER BY token").bind(child).bind(org.0).bind(purpose).fetch_all(&mut *conn).await?;
...
pub(crate) async fn control(
    ...
    let r=sqlx::query("SELECT i.snapshot_id,i.measured_bytes-i.retained_bytes AS delta,r.token,r.byte_count FROM migration_admitted_activity_import i JOIN migration_admitted_activity_reservation r ON r.import_id=i.id AND r.organization_id=i.organization_id AND r.purpose='cancel' WHERE i.id=$1 AND i.organization_id=$2 FOR UPDATE OF r").bind(child).bind(org.0).fetch_optional(&mut *conn).await?.ok_or(MigrationError::StorageLimit)?;
    let delta: i64 = r.get("delta");
    let available: i64 = r.get("byte_count");
    if delta >= available {
        return Err(MigrationError::StorageLimit);
    }
    sqlx::query("UPDATE migration_admitted_activity_reservation SET byte_count=byte_count-$3 WHERE token=$1 AND organization_id=$2").bind(r.get::<Uuid,_>("token")).bind(org.0).bind(delta).execute(&mut *conn).await?;
    sqlx::query("UPDATE migration_admitted_activity_import SET reserved_bytes=reserved_bytes-$3,retained_bytes=retained_bytes+$3 WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).bind(delta).execute(&mut *conn).await?;
...
pub(crate) async fn charge(
    ...
    let amount:i64=sqlx::query_scalar("SELECT measured_bytes-retained_bytes FROM migration_admitted_activity_import WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).fetch_one(&mut *conn).await?;
    if amount <= 0 {
        sqlx::query("UPDATE migration_admitted_activity_import SET retained_bytes=retained_bytes+$3 WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).bind(amount).execute(&mut *conn).await?;
...
pub(crate) async fn replay<T: Serialize>(
    ...
    let r=sqlx::query("SELECT * FROM migration_admitted_activity_receipt WHERE organization_id=$1 AND actor_user_id=$2 AND action=$3 AND request_id=$4").bind(ctx.organization_id.0).bind(ctx.actor_user_id.0).bind(action).bind(request).fetch_optional(conn).await?;
...
pub(crate) async fn receipt<T: Serialize>(
    ...
    let pending:i64=sqlx::query_scalar("SELECT measured_bytes-retained_bytes FROM migration_admitted_activity_import WHERE id=$1 AND organization_id=$2").bind(child).bind(ctx.organization_id.0).fetch_one(&mut *conn).await?;
