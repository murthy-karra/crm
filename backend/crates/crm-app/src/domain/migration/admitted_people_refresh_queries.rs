...
pub async fn detail(
    ...
    let remainder = row.get::<String, _>("state") == "cancelled"
        ...
        && row
            ...
            .is_some();
    value["mapping_repair_available"]=json!(sqlx::query_scalar::<_,bool>("SELECT (NOT $3 AND (EXISTS(SELECT 1 FROM migration_admitted_people_refresh_item i WHERE i.refresh_id=$1 AND i.organization_id=$2 AND i.disposition='held_mapping_gap') OR EXISTS(SELECT 1 FROM migration_admitted_people_refresh_result z WHERE z.refresh_id=$1 AND z.organization_id=$2 AND z.disposition IN ('held_stale','held_mapping_gap')))) OR ($3 AND EXISTS(SELECT 1 FROM migration_admitted_people_refresh_item i WHERE i.refresh_id=$1 AND i.organization_id=$2 AND i.plan_id=$4 AND i.settled_at IS NULL AND i.disposition IN ('eligible','already_current')))").bind(id).bind(ctx.organization_id.0).bind(remainder).bind(row.get::<Option<Uuid>,_>("confirmed_refresh_plan_id")).fetch_one(&mut *tx).await?);
...
pub async fn item(
    ...
    let stages = sqlx::query(
        "SELECT id,left(name,256) AS label FROM stage WHERE organization_id=$1 AND id=ANY($2)",
    )
    .bind(ctx.organization_id.0)
    .bind(ids("stage_id"))
    .fetch_all(&mut *tx)
    ...
    for side in ["baseline", "current", "proposed"] {
        for (field, label, rows) in [
            ("stage_id", "current_stage_label", &stages),
            ("assigned_user_id", "current_assignee_label", &assignees),
        ] {
            let id = value[side][field]
                .as_str()
                .and_then(|v| Uuid::parse_str(v).ok());
            value[side][label] = json!(rows
                .iter()
                .find(|row| Some(row.get::<Uuid, _>("id")) == id)
