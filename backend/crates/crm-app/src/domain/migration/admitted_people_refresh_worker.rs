...
pub async fn run_once(
    ...
    let r = sqlx::query(CANDIDATE_SQL).fetch_optional(pool).await?;
...
async fn execute_noop(
    ...
    sqlx::query("UPDATE migration_admitted_people_refresh SET settled_items=settled_items+1 WHERE id=$1 AND organization_id=$2").bind(id).bind(org.0).execute(&mut *conn).await?;
    if disposition == "settled" {
...
pub(super) const CANDIDATE_SQL: &str = "SELECT id,organization_id,lease_token FROM migration_admitted_people_refresh WHERE state IN ('preparing','queued','running') ORDER BY created_at,id LIMIT 1";
