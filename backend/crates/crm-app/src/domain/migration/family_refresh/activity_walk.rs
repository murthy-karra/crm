//! One retained activity occurrence per transaction outcome. Equal occurrences
//! share a unit; source conflicts are resolved before frozen-cohort filtering.
use super::{
    activity_plan::{self, Prepared},
    cohort::Claim,
...
use crate::{
    ...
    domain::migration::{activity_store, crypto, snapshot::SnapshotPolicy, MigrationError},
};
use serde::Serialize;
...
pub async fn run_once(
    ...
    let row=sqlx::query("SELECT s.*,owner.revision AS source_revision,owner.phase AS source_phase,owner.family AS source_family FROM migration_family_refresh_source s JOIN migration_family_refresh_plan owner ON owner.id=s.plan_id AND owner.bundle_id=s.bundle_id AND owner.organization_id=s.organization_id WHERE s.bundle_id=$1 AND s.organization_id=$2 AND s.kind IN ('note','task') AND ($3::uuid IS NULL OR s.id>$3) ORDER BY s.id LIMIT 1").bind(claim.bundle).bind(claim.organization.0).bind(before).fetch_optional(&mut *tx).await?;
    ...
    let (kind, source_kind) = match row.get::<String, _>("kind").as_str() {
        "note" => (Kind::Note, core_resolution::Kind::Note),
        "task" => (Kind::Task, core_resolution::Kind::Task),
    ...
    match &data {
        Derived::Activity(record)
            if record.source_id == source_id
                && record.person_id == row.get::<Option<String>, _>("source_person_id")
                && record.stream.representation() == row.get::<String, _>("representation") => {}
        Derived::Oversized { .. }
    ...
    let hash = if let Some(source) = source_id.as_ref() {
        ...
        crypto::snapshot_hmac(
            key,
            claim.organization,
            "family-refresh-activity-diagnostic-v1",
    ...
    let held = if let Some(source) = source_id.as_deref() {
        match core_resolution::resolve(pool, key, claim, source_kind, source).await? {
            Resolution::Ready(selected) => {
                let (mut tx, _, _) = preparation::begin(pool, claim).await?;
                let cohort:Option<Uuid>=sqlx::query_scalar("SELECT id FROM migration_family_refresh_cohort WHERE bundle_id=$1 AND organization_id=$2 AND source_person_id=$3").bind(claim.bundle).bind(claim.organization.0).bind(&selected.source_person).fetch_optional(&mut *tx).await?;
                drop(tx);
                if let Some(cohort) = cohort {
                    match activity_plan::prepare(
                        pool,
                        key,
                        policy,
                        claim,
        ...
        if matches!(data, Derived::Oversized { .. }) {
            Hold::UnitTooLarge
        } else {
...
async fn persist(
    ...
    let kind = if diagnostic.kind == Kind::Note {
        "note"
    } else {
        "task"
    };
    ...
    advance(&mut tx, claim, Some(position)).await?;
    tx.commit().await?;
    tracing::info!(organization_id=%claim.organization,plan_id=%claim.plan,manifest_id=%id,excluded=diagnostic.excluded,"Family refresh activity diagnostic prepared");
