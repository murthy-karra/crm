...
pub async fn apply_once(
    ...
    let Some(unit) = unit else {
        ...
        sqlx::query("UPDATE migration_family_refresh_bundle SET state=CASE WHEN EXISTS(SELECT 1 FROM migration_family_refresh_plan WHERE bundle_id=$1 AND organization_id=$2 AND state IN ('queued','running')) THEN 'running' WHEN EXISTS(SELECT 1 FROM migration_family_refresh_plan WHERE bundle_id=$1 AND organization_id=$2 AND state='paused') THEN 'paused' WHEN EXISTS(SELECT 1 FROM migration_family_refresh_plan WHERE bundle_id=$1 AND organization_id=$2 AND state='cancelled' AND confirmed_at IS NOT NULL) THEN 'cancelled' ELSE 'completed' END,updated_at=clock_timestamp() WHERE id=$1 AND organization_id=$2").bind(claim.bundle).bind(claim.organization.0).execute(&mut *tx).await?;
