...
async fn step(
    ...
    if !p.get::<bool, _>("remainder_complete") {
        drop(tx);
        return super::remainder_copy::run_once(pool, key, policy, claim).await;
    }
    ...
    {
        let progress = if p.get::<bool, _>("source_walk_complete") {
            super::activity_missing::run_once(pool, key, policy, claim).await?
        } else {
            super::activity_walk::run_once(pool, key, policy, claim).await?
        };
        return Ok(progress != super::activity_walk::Progress::Capacity);
    }
    if family == "metadata"
        && p.get::<bool, _>("mappings_complete")
        && matches!(phase.as_str(), "mappings" | "classify")
    {
        if !p.get::<bool, _>("catalog_walk_complete") {
            return Ok(
                super::catalog_walk::run_once(pool, key, policy, claim).await?
                    != super::catalog_walk::Progress::Capacity,
            );
        }
        let progress = if !p.get::<bool, _>("source_walk_complete") {
            super::metadata_walk::run_once(pool, key, policy, claim).await?
        } else {
            super::metadata_walk::missing_once(pool, key, policy, claim).await?
        };
        return Ok(progress != super::metadata_walk::Progress::Capacity);
    }
