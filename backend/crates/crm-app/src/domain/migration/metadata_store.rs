//! Child-owned storage and encryption. No source/parent reservation is released here.
...
pub(crate) fn source_key(
    ...
    crypto::snapshot_hmac(key, org, &format!("metadata-key-v1:{account}:{kind}"), raw).to_vec()
}
...
pub(crate) async fn plan(
    ...
    sqlx::query("SELECT * FROM migration_metadata_plan WHERE id=$1 AND import_id=$2 AND organization_id=$3 FOR UPDATE").bind(id).bind(child).bind(org.0).fetch_optional(conn).await?.ok_or(MigrationError::NotFound)
}
#[allow(clippy::too_many_arguments)] // Keep tenant, immutable evidence and lease/receipt scopes explicit.
pub(crate) async fn reserve(
    ...
    sqlx::query("UPDATE migration_metadata_import SET reserved_bytes=reserved_bytes+$3 WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).bind(amount).execute(conn).await?;
    Ok(true)
}
pub(crate) async fn settle(
    ...
    let r=sqlx::query("DELETE FROM migration_metadata_reservation WHERE token=$1 AND import_id=$2 AND organization_id=$3 RETURNING byte_count,snapshot_id").bind(token).bind(child).bind(org.0).fetch_optional(&mut *conn).await?.ok_or(MigrationError::ImportConflict)?;
    ...
    sqlx::query("UPDATE migration_metadata_import SET reserved_bytes=reserved_bytes-$3,retained_bytes=retained_bytes+$4 WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).bind(reserved).bind(actual).execute(conn).await?;
    Ok(())
}
pub(crate) async fn release(
    ...
    let ids=sqlx::query_scalar::<_,Uuid>("SELECT token FROM migration_metadata_reservation WHERE import_id=$1 AND organization_id=$2 AND purpose=$3 ORDER BY token").bind(child).bind(org.0).bind(purpose).fetch_all(&mut *conn).await?;
...
pub(crate) async fn charge(
    ...
    if amount <= 0 {
        sqlx::query("UPDATE migration_metadata_import SET retained_bytes=retained_bytes+$3 WHERE id=$1 AND organization_id=$2").bind(child).bind(org.0).bind(amount).execute(&mut *conn).await?;
...
pub(crate) async fn replay<T: Serialize>(
    ...
    let digest = crypto::snapshot_hmac(
        key,
        ctx.organization_id,
        &format!("metadata-request:{}:{action}", ctx.actor_user_id.0),
        &bytes(input)?,
    );
    let r=sqlx::query("SELECT * FROM migration_metadata_receipt WHERE organization_id=$1 AND actor_user_id=$2 AND action=$3 AND request_id=$4").bind(ctx.organization_id.0).bind(ctx.actor_user_id.0).bind(action).bind(request).fetch_optional(conn).await?;
    ...
    let raw = crypto::open_receipt(
        ...
        ctx.organization_id,
        request,
        &format!("metadata:{}:{action}", ctx.actor_user_id.0),
    ...
    if action == "cancel" {
        ...
        for name in ["run_reserved_bytes", "org_reserved_bytes"] {
            ...
            response["import"]["policy"][name] = serde_json::json!(before
                .checked_sub(CANCEL_RESERVATION)
                .ok_or(MigrationError::Crypto)?
                .to_string());
        }
    }
