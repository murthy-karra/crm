...
async fn selected_plan(
    ...
    let row=sqlx::query("SELECT * FROM migration_people_admission_plan WHERE admission_id=$1 AND organization_id=$2 AND ($3::uuid IS NULL OR id=$3) ORDER BY revision DESC LIMIT 1").bind(admission).bind(ctx.organization_id.0).bind(requested).fetch_optional(conn).await?.ok_or(MigrationError::NotFound)?;
...
pub async fn list(
    ...
    let parent = page.parent_import_id.ok_or(MigrationError::InvalidInput)?;
    ...
    let rows=sqlx::query("SELECT * FROM migration_people_admission WHERE organization_id=$1 AND parent_import_id=$2 AND (created_at,id)<=($3,$4) AND ($5::timestamptz IS NULL OR (created_at,id)<($5,$6)) ORDER BY created_at DESC,id DESC LIMIT $7")
    ...
    finish(
        tx,
        json!({"admissiones":values,"next_cursor":next}),
        SUMMARY_BYTES,
    )
    .await
}
#[tracing::instrument(skip_all, fields(organization_id=%ctx.organization_id.0,admission_id=%id))]
pub async fn detail(
    pool: &PgPool,
    key: &RawPayloadKey,
    ctx: &CommandContext,
    ...
    let plan=sqlx::query("SELECT * FROM migration_people_admission_plan WHERE admission_id=$1 AND organization_id=$2 ORDER BY revision DESC LIMIT 1").bind(id).bind(ctx.organization_id.0).fetch_optional(&mut *tx).await?;
...
pub async fn items(
    ...
    let cursor = decode(key, ctx, &tag, page.cursor.as_deref())?;
    let rows=sqlx::query("SELECT id,source_id,person_id,disposition,name_clear_count,assignment_clear_count,contact_removal_count,instructions_nonce,instructions_ciphertext FROM migration_people_admission_item WHERE admission_id=$1 AND organization_id=$2 AND plan_id=$3 AND id>$4 AND ($5::text IS NULL OR disposition=$5) ORDER BY id LIMIT $6")
...
async fn item_row(
    ...
    let metadata=sqlx::query("SELECT plan_id,octet_length(baseline_ciphertext)+octet_length(current_ciphertext)+octet_length(proposed_ciphertext)+octet_length(instructions_ciphertext) AS bytes FROM migration_people_admission_item WHERE id=$1 AND admission_id=$2 AND organization_id=$3")
        .bind(item).bind(admission).bind(ctx.organization_id.0).fetch_optional(&mut *conn).await?.ok_or(MigrationError::NotFound)?;
    if i64::from(metadata.get::<i32, _>("bytes")) > s::ITEM_LIMIT {
        return Err(MigrationError::StorageLimit);
    }
    let plan = selected_plan(conn, ctx, admission, Some(metadata.get("plan_id"))).await?;
    let row=sqlx::query("SELECT * FROM migration_people_admission_item WHERE id=$1 AND admission_id=$2 AND organization_id=$3").bind(item).bind(admission).bind(ctx.organization_id.0).fetch_one(conn).await?;
    Ok((row, plan))
}
fn projection(
    key: &RawPayloadKey,
...
pub async fn contacts(
    ...
    let count = limit(&page, 50)?;
    if page.parent_import_id.is_some() || page.plan_id.is_some() || page.disposition.is_some() {
        return Err(MigrationError::InvalidInput);
    }
    let mut tx = begin(pool, ctx).await?;
    resource(&mut tx, key, ctx, id).await?;
    let plan_id:Uuid=sqlx::query_scalar("SELECT plan_id FROM migration_people_admission_item WHERE id=$1 AND admission_id=$2 AND organization_id=$3").bind(item).bind(id).bind(ctx.organization_id.0).fetch_optional(&mut *tx).await?.ok_or(MigrationError::NotFound)?;
    let plan = selected_plan(&mut tx, ctx, id, Some(plan_id)).await?;
    let tag = binding(ctx, item, "contacts", Some(&plan), None, count);
    let cursor = decode(key, ctx, &tag, page.cursor.as_deref())?;
    let rows=sqlx::query("SELECT c.*,CASE c.side WHEN 'current' THEN 'current' WHEN 'baseline' THEN CASE WHEN EXISTS(SELECT 1 FROM migration_people_admission_contact p WHERE p.organization_id=c.organization_id AND p.admission_id=c.admission_id AND p.item_id=c.item_id AND p.side='proposed' AND p.contact_id=c.contact_id) THEN 'retain' ELSE 'remove' END WHEN 'proposed' THEN CASE WHEN EXISTS(SELECT 1 FROM migration_people_admission_contact b WHERE b.organization_id=c.organization_id AND b.admission_id=c.admission_id AND b.item_id=c.item_id AND b.side='baseline' AND b.contact_id=c.contact_id) THEN 'retain' ELSE 'add' END END AS change FROM migration_people_admission_contact c WHERE c.admission_id=$1 AND c.organization_id=$2 AND c.item_id=$3 AND c.id>$4 ORDER BY c.id LIMIT $5")
...
pub async fn results(
    ...
    let upper = if let Some(cursor) = &cursor {
        ...
        let last=sqlx::query("SELECT id,committed_at FROM migration_people_admission_result WHERE admission_id=$1 AND organization_id=$2 ORDER BY committed_at DESC,id DESC LIMIT 1").bind(id).bind(ctx.organization_id.0).fetch_optional(&mut *tx).await?;
    ...
    let rows=sqlx::query("SELECT id,item_id,person_id,source_id,disposition,committed_at FROM migration_people_admission_result WHERE admission_id=$1 AND organization_id=$2 AND (committed_at,id)<=($3,$4) AND ($5::timestamptz IS NULL OR (committed_at,id)>($5,$6)) ORDER BY committed_at,id LIMIT $7")
...
/// Admin-only review read for the distinct 010e3 provenance origin.  It is
/// intentionally separate from the original-import provenance projection.
pub async fn admission_provenance(
    ...
    let mut tx = begin(pool, ctx).await?;
    let row = sqlx::query("SELECT p.*,a.id admission_id,a.parent_import_id,a.parent_plan_id,a.newer_snapshot_id,a.original_snapshot_id,i.id item_id FROM person_admission_provenance p JOIN migration_people_admission a ON a.id=p.admission_id AND a.organization_id=p.organization_id JOIN migration_people_admission_item i ON i.id=p.item_id AND i.admission_id=a.id AND i.organization_id=a.organization_id WHERE p.organization_id=$1 AND p.person_id=$2")
        .bind(ctx.organization_id.0).bind(person).fetch_optional(&mut *tx).await?.ok_or(MigrationError::NotFound)?;
    ...
    tx.commit().await?;
    Ok(
        json!({"person_id":person,"admission_id":row.get::<Uuid,_>("admission_id"),"item_id":row.get::<Uuid,_>("item_id"),"parent_import_id":row.get::<Uuid,_>("parent_import_id"),"parent_plan_id":row.get::<Uuid,_>("parent_plan_id"),"original_snapshot_id":row.get::<Uuid,_>("original_snapshot_id"),"newer_snapshot_id":row.get::<Uuid,_>("newer_snapshot_id"),"coverage":"core_only_notes_tasks_metadata_history_deferred","provenance":value}),
    )
}
pub async fn admission_provenance_contacts(
    ...
    let mut tx = begin(pool, ctx).await?;
    let maximum = limit(&page, 50)?;
    let row=sqlx::query("SELECT p.admission_id,p.item_id FROM person_admission_provenance p WHERE p.organization_id=$1 AND p.person_id=$2").bind(ctx.organization_id.0).bind(person).fetch_optional(&mut *tx).await?.ok_or(MigrationError::NotFound)?;
    let rows=sqlx::query("SELECT * FROM migration_people_admission_contact WHERE admission_id=$1 AND item_id=$2 ORDER BY id LIMIT $3").bind(row.get::<Uuid,_>("admission_id")).bind(row.get::<Uuid,_>("item_id")).bind(maximum).fetch_all(&mut *tx).await?;
    let mut values = Vec::new();
    for c in rows {
        let value: String = s::open(
            key,
            ctx.organization_id,
            row.get("admission_id"),
            c.get("id"),
            "contact",
            &c.get::<Vec<u8>, _>("value_nonce"),
            &c.get::<Vec<u8>, _>("value_ciphertext"),
        )?;
        values.push(json!({"kind":c.get::<String,_>("kind"),"import_order":c.get::<i32,_>("import_order").to_string(),"value":value,"normalized_value":c.get::<String,_>("normalized_value"),"primary":c.get::<bool,_>("primary_contact")}))
    }
    tx.commit().await?;
    Ok(json!({"items":values,"next_cursor":Value::Null}))
}
pub async fn admission_provenance_field(
    ...
    let value = admission_provenance(pool, key, ctx, person).await?;
    let text = if field == "source_id" {
        value["provenance"]["source_id"]
            .as_str()
            .unwrap_or_default()
    } else {
    ...
    Ok(
        json!({"field":field,"offset":"0","total_bytes":text.len().to_string(),"fragment":text,"next_cursor":Value::Null}),
    )
}
