...
use super::{
    core_change_store, history_capture_store, people_refresh_store as s, store, MigrationError,
};
...
async fn resource(
    ...
    let row = sqlx::query(
        "SELECT * FROM migration_people_refresh WHERE id=$1 AND organization_id=$2 FOR SHARE",
    )
    .bind(id)
    .bind(ctx.organization_id.0)
...
async fn selected_plan(
    ...
    let row=sqlx::query("SELECT * FROM migration_people_refresh_plan WHERE refresh_id=$1 AND organization_id=$2 AND ($3::uuid IS NULL OR id=$3) ORDER BY revision DESC LIMIT 1").bind(refresh).bind(ctx.organization_id.0).bind(requested).fetch_optional(conn).await?.ok_or(MigrationError::NotFound)?;
...
pub async fn list(
    ...
    let rows=sqlx::query("SELECT * FROM migration_people_refresh WHERE organization_id=$1 AND parent_import_id=$2 AND (created_at,id)<=($3,$4) AND ($5::timestamptz IS NULL OR (created_at,id)<($5,$6)) ORDER BY created_at DESC,id DESC LIMIT $7")
...
pub async fn detail(
    ...
    let plan=sqlx::query("SELECT * FROM migration_people_refresh_plan WHERE refresh_id=$1 AND organization_id=$2 ORDER BY revision DESC LIMIT 1").bind(id).bind(ctx.organization_id.0).fetch_optional(&mut *tx).await?;
...
pub async fn items(
    ...
    let cursor = decode(key, ctx, &tag, page.cursor.as_deref())?;
    let rows=sqlx::query("SELECT id,source_id,person_id,disposition,name_clear_count,assignment_clear_count,contact_removal_count,instructions_nonce,instructions_ciphertext FROM migration_people_refresh_item WHERE refresh_id=$1 AND organization_id=$2 AND plan_id=$3 AND id>$4 AND ($5::text IS NULL OR disposition=$5) ORDER BY id LIMIT $6")
...
async fn item_row(
    ...
    let metadata=sqlx::query("SELECT plan_id,octet_length(baseline_ciphertext)+octet_length(current_ciphertext)+octet_length(proposed_ciphertext)+octet_length(instructions_ciphertext) AS bytes FROM migration_people_refresh_item WHERE id=$1 AND refresh_id=$2 AND organization_id=$3")
    ...
    let row=sqlx::query("SELECT * FROM migration_people_refresh_item WHERE id=$1 AND refresh_id=$2 AND organization_id=$3").bind(item).bind(refresh).bind(ctx.organization_id.0).fetch_one(conn).await?;
    Ok((row, plan))
}
fn projection(
    key: &RawPayloadKey,
...
pub async fn results(
    ...
    let upper = if let Some(cursor) = &cursor {
        ...
        let last=sqlx::query("SELECT id,committed_at FROM migration_people_refresh_result WHERE refresh_id=$1 AND organization_id=$2 ORDER BY committed_at DESC,id DESC LIMIT 1").bind(id).bind(ctx.organization_id.0).fetch_optional(&mut *tx).await?;
    ...
    let rows=sqlx::query("SELECT id,item_id,person_id,source_id,disposition,committed_at FROM migration_people_refresh_result WHERE refresh_id=$1 AND organization_id=$2 AND (committed_at,id)<=($3,$4) AND ($5::timestamptz IS NULL OR (committed_at,id)>($5,$6)) ORDER BY committed_at,id LIMIT $7")
