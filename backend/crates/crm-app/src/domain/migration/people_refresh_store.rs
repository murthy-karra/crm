...
pub async fn prepared_bytes(
    ...
    sqlx::query_scalar("SELECT COALESCE(sum(prepared_bytes),0)::bigint FROM migration_people_refresh_plan WHERE refresh_id=$1 AND organization_id=$2")
...
pub async fn measured_bytes(
    ...
    let base: i64 = sqlx::query_scalar(
        "SELECT
          COALESCE((SELECT sum(octet_length(inputs_nonce)+octet_length(inputs_ciphertext)+COALESCE(octet_length(digest),0)) FROM migration_people_refresh_plan WHERE refresh_id=$1 AND organization_id=$2),0)
        + COALESCE((SELECT sum(octet_length(proposed_nonce)+octet_length(proposed_ciphertext)+octet_length(baseline_nonce)+octet_length(baseline_ciphertext)+octet_length(current_nonce)+octet_length(current_ciphertext)+octet_length(instructions_nonce)+octet_length(instructions_ciphertext)) FROM migration_people_refresh_item WHERE refresh_id=$1 AND organization_id=$2),0)
        + COALESCE((SELECT sum(octet_length(value_nonce)+octet_length(value_ciphertext)) FROM migration_people_refresh_contact WHERE refresh_id=$1 AND organization_id=$2),0)
        + COALESCE((SELECT sum(octet_length(before_nonce)+octet_length(before_ciphertext)+octet_length(after_nonce)+octet_length(after_ciphertext)) FROM migration_people_refresh_result WHERE refresh_id=$1 AND organization_id=$2),0)
        + COALESCE((SELECT sum(octet_length(nonce)+octet_length(ciphertext)) FROM migration_people_refresh_receipt WHERE refresh_id=$1 AND organization_id=$2),0)
        + COALESCE((SELECT sum(octet_length(projection_nonce)+octet_length(projection_ciphertext)) FROM migration_people_refresh_baseline WHERE refresh_id=$1 AND organization_id=$2),0)",
    )
    .bind(refresh)
    .bind(org.0)
    .fetch_one(&mut *conn)
...
pub async fn reserve(
    ...
    let row = sqlx::query(
        "SELECT r.newer_snapshot_id,s.run_byte_limit,s.retained_bytes,s.reserved_bytes,
           ...
           FROM migration_people_refresh r
    ...
    sqlx::query("INSERT INTO migration_people_refresh_reservation(token,refresh_id,organization_id,purpose,lease_token,byte_count) VALUES($1,$2,$3,$4,$5,$6)")
        .bind(token).bind(refresh).bind(org.0).bind(purpose).bind(lease).bind(amount)
        .execute(&mut *conn).await?;
    sqlx::query("UPDATE migration_people_refresh SET reserved_bytes=reserved_bytes+$3 WHERE id=$1 AND organization_id=$2")
...
pub async fn release(
    ...
    let amount: i64 = sqlx::query_scalar(
        "DELETE FROM migration_people_refresh_reservation
    ...
    let snapshot: Uuid = sqlx::query_scalar(
        "SELECT newer_snapshot_id FROM migration_people_refresh WHERE id=$1 AND organization_id=$2",
    )
    .bind(refresh)
    .bind(org.0)
    .fetch_one(&mut *conn)
    .await?;
    sqlx::query("UPDATE migration_people_refresh SET reserved_bytes=reserved_bytes-$3,retained_bytes=retained_bytes+$4 WHERE id=$1 AND organization_id=$2")
...
pub async fn debit_baseline_owner(
    ...
    let snapshot: Uuid = sqlx::query_scalar(
        "SELECT newer_snapshot_id FROM migration_people_refresh WHERE id=$1 AND organization_id=$2 FOR UPDATE",
    )
    .bind(prior_refresh)
    .bind(org.0)
    .fetch_one(&mut *conn)
    ...
    let changed = sqlx::query(
        "UPDATE migration_people_refresh SET retained_bytes=retained_bytes-$3
