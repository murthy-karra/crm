...
// Admission is additive. A new binary may still serve ordinary reads on a
// pre-admission schema, before the separately gated feature is enabled.
async fn admitted_history(
    conn: &mut PgConnection,
    org: OrganizationId,
    person: PersonId,
) -> Result<Vec<HistoryEntry>, sqlx::Error> {
    use sqlx::Row;
    let present: bool =
        sqlx::query_scalar("SELECT to_regclass('public.person_admitted') IS NOT NULL")
            .fetch_one(&mut *conn)
            .await?;
    if !present {
        return Ok(Vec::new());
    }
    let rows = sqlx::query("SELECT id,occurred_at,recorded_at,correlation_id,on_behalf_of_user_id,admission_id,plan_id,item_id,result_id FROM person_admitted WHERE organization_id=$1 AND person_id=$2 ORDER BY occurred_at,id LIMIT 2")
        .bind(org.0).bind(person.0).fetch_all(conn).await?;
    if rows.len() > 1 {
        return Err(sqlx::Error::Protocol(
            "inconsistent admission history".into(),
        ));
    }
    Ok(rows
        .into_iter()
