...
pub enum TodaySourceIssueError {
    ...
    /// during Today's per-source evaluation (compile-forced by
    /// `FilterError`/`SourceEvaluation` gaining `InvalidTag`; the
    /// deleted-before-enumeration wildcard sites in `today/mod.rs` are e2
    /// step 3).
    InvalidTag,
    Unavailable,
}
