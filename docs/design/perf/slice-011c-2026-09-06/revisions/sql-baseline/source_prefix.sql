-- Bounded 011c prototype: select and hydrate a <= K+1 non-B source prefix in
-- one source savepoint. The predicate matrix is copied from current
-- crm-app/src/domain/person/queries.rs::filtered_summaries, with $21 replacing
-- now() for the request's single evaluation timestamp. $22 excludes B and $23
-- is K+1. Hydration runs inside this statement so an error/time budget rolls the
-- full source contribution back before it is merged with other sources.
WITH prefix AS (
