...
const BASELINE_BUILTIN_SQL: &str = include_str!("../revisions/sql-baseline/builtin.sql");
const BASELINE_SOURCE_PREFIX_SQL: &str =
    include_str!("../revisions/sql-baseline/source_prefix.sql");
...
async fn run_mode(pool: sqlx::PgPool, mode: &str) -> Vec<Summary> {
    ...
    let remaining = cases.iter().skip(4).cloned().collect::<Vec<_>>();
...
async fn capture_plans(
    ...
    let builtin_plan: Json<serde_json::Value> = sqlx::query(&builtin_explain)
        ...
        .await?
        .try_get(0)?;
    let suffix = env::var("PERF_PLAN_LABEL").unwrap_or_else(|_| {
        if disable_jit {
            "jit_off".to_string()
        } else {
            "default".to_string()
...
async fn builtin_payloads(
    ...
    let wrapped = format!(
        "SELECT row_to_json(candidate)::text AS payload FROM ({sql}) AS candidate \
         ORDER BY (candidate.\"by_inquiry?\" OR candidate.\"client_replied_occurred_at?\" IS NOT NULL) DESC, \
                  candidate.\"fresh?\" DESC, candidate.\"waiting_since?\" ASC, candidate.id ASC"
    );
    sqlx::query(&wrapped)
        .persistent(true)
        .bind(org())
...
async fn source_payloads(
    ...
    let wrapped = format!(
        "SELECT row_to_json(candidate)::text AS payload FROM ({sql}) AS candidate \
         ORDER BY (candidate.last_contact_at IS NOT NULL) ASC, candidate.last_contact_at ASC, candidate.id ASC"
    );
    bind_filter(sqlx::query(&wrapped), filter, now, builtin_ids)
        .bind(101_i64)
        .fetch_all(&mut **tx)
        .await?
        .into_iter()
...
async fn verify_parity(
    ...
    let mut builtin_cases = Vec::new();
    let viewers = [
        ("concentrated_spare_B", Uuid::parse_str(CONCENTRATED)?),
        ("representative_full_B", Uuid::parse_str(REPRESENTATIVE)?),
        ("partial_B", Uuid::parse_str(PARTIAL)?),
        ("empty_B", Uuid::parse_str(EMPTY)?),
    ];
    let mut concentrated_b = Vec::new();
    for (label, viewer) in viewers {
        let baseline = builtin_payloads(&mut tx, BASELINE_BUILTIN_SQL, viewer, now).await?;
        let candidate = builtin_payloads(&mut tx, BUILTIN_SQL, viewer, now).await?;
        if baseline != candidate {
            return Err(std::io::Error::other(format!(
                "builtin payload/order mismatch for {label}"
            ))
            .into());
        }
        if label == "concentrated_spare_B" {
            concentrated_b = sqlx::query(BASELINE_BUILTIN_SQL)
                .persistent(true)
                .bind(org())
                .bind(viewer)
                .bind(now)
                ...
                .map(|row| row.try_get::<Uuid, _>("id"))
                .collect::<Result<Vec<_>, _>>()?;
            concentrated_b.sort_unstable();
        }
        builtin_cases.push(serde_json::json!({"case": label, "rows": baseline.len()}));
    }

    let mut source_cases = Vec::new();
    let concentrated = Uuid::parse_str(CONCENTRATED)?;
    for source_case in ["one_dense", "one_absence", "five_overlap"] {
        let rows = sqlx::query(METADATA_SQL)
            .bind(org())
            .bind(concentrated)
            .bind(source_case)
            .fetch_all(&mut *tx)
        ...
        for (position, row) in rows.into_iter().enumerate() {
            let value: Json<serde_json::Value> = row.try_get("filter")?;
            let definition = serde_json::from_value::<FilterDefinition>(value.0)?;
            definition
                .validate()
                .expect("seeded definition must validate");
            definition
                .validate_references(&mut *tx, OrganizationId::new(org()))
                .await
                .expect("seeded references must validate");
            let filter = Filter::from_saved(
                definition.kinds_field(),
                definition.to_query_params(UserId::new(concentrated)),
            );
            let baseline = source_payloads(
                &mut tx,
                BASELINE_SOURCE_PREFIX_SQL,
                &filter,
                now,
                &concentrated_b,
            )
            .await?;
            let candidate =
                source_payloads(&mut tx, SOURCE_PREFIX_SQL, &filter, now, &concentrated_b).await?;
            if baseline != candidate {
                return Err(std::io::Error::other(format!(
                    "source payload/order mismatch for {source_case}:{position}:{}",
                    filter.label
                ))
                .into());
            }
            source_cases.push(serde_json::json!({
                "case": source_case,
                "position": position + 1,
                "label": filter.label,
                "rows": baseline.len()
    ...
    let me_definition = serde_json::from_str::<FilterDefinition>(
        r#"{"version":1,"clauses":[{"kind":"stage","stage_ids":["22222222-2222-4222-8222-222222222222"]},{"kind":"assigned_to","assignees":["me"]},{"kind":"source","sources":["zillow"]}]}"#,
    )?;
    me_definition
        .validate()
        .expect("narrow me definition must validate");
    me_definition
        .validate_references(&mut *tx, OrganizationId::new(org()))
        .await
        .expect("narrow me references must validate");
    let me_filter = Filter::from_saved(
        "narrow_me_source".to_string(),
        me_definition.to_query_params(UserId::new(concentrated)),
    );
    let me_baseline = source_payloads(
        &mut tx,
        BASELINE_SOURCE_PREFIX_SQL,
        &me_filter,
        now,
    ...
    .await?;
    let me_candidate =
        source_payloads(&mut tx, SOURCE_PREFIX_SQL, &me_filter, now, &concentrated_b).await?;
    if me_baseline != me_candidate {
        return Err(
            std::io::Error::other("source payload/order mismatch for narrow_me_source").into(),
        );
    }
    source_cases.push(serde_json::json!({
        "case": "narrow_me_source",
        "position": 1,
        "label": me_filter.label,
        "rows": me_baseline.len()
    }));
    tx.commit().await?;
    Ok(serde_json::json!({
        "result": "equal",
        "transaction": "one read-only repeatable-read snapshot with one server transaction_timestamp",
        "comparison": "full selected row payload plus explicit Today/source order; source output includes last_contact_at sort key",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if mode == "parity" {
        let report = verify_parity(&pool).await?;
        fs::write(
            artifact_dir.join("metrics").join("parity-candidate.json"),
    ...
    if mode == "five_c20_raw" {
        let current = all_cases()
            .into_iter()
            .nth(3)
            .expect("concentrated five-source case exists");
        warm(pool.clone(), current.clone()).await;
        let mut raw = Vec::new();
        for _ in 0..2 {
            raw.extend(wave(pool.clone(), current.clone(), 20).await);
        }
        fs::write(
            artifact_dir
                .join("metrics")
                .join("five-source-c20-raw-candidate-guards-jit-off.json"),
