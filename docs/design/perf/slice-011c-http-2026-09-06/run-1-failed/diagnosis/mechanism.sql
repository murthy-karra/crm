...
PREPARE builtin(uuid, uuid, timestamptz) AS WITH outcome_call AS (
           ...
           ORDER BY (membership.by_inquiry OR reply_membership.by_reply) DESC,
                    ...
                    p.id ASC
           LIMIT 201;
PREPARE original(uuid, uuid, timestamptz) AS WITH outcome_call AS (
               SELECT DISTINCT ON (c.person_id) c.person_id, c.id, c.ended_at
               FROM call c
               WHERE c.organization_id = $1
                 AND c.caller_user_id = $2
           ...
           ORDER BY (membership.by_inquiry OR reply_membership.by_reply) DESC,
                    ...
                    p.id ASC
           LIMIT 201;
\echo === A: mergejoin off, jit off (candidate transaction-local policy) x3 ===
SET jit = off; SET enable_mergejoin = off;
...
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF) EXECUTE builtin('40ca6bb3-5e13-487f-859a-bef64207a330', '79b710c9-f440-4299-aa35-6982e466ade0', '2026-09-06T12:00:00Z');
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF) EXECUTE builtin('40ca6bb3-5e13-487f-859a-bef64207a330', '79b710c9-f440-4299-aa35-6982e466ade0', '2026-09-06T12:00:00Z');
\echo === B: frozen original query, jit on, default planner (slow regime) x2 ===
RESET enable_mergejoin; SET jit = on;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF) EXECUTE original('40ca6bb3-5e13-487f-859a-bef64207a330', '79b710c9-f440-4299-aa35-6982e466ade0', '2026-09-06T12:00:00Z');
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF) EXECUTE original('40ca6bb3-5e13-487f-859a-bef64207a330', '79b710c9-f440-4299-aa35-6982e466ade0', '2026-09-06T12:00:00Z');
\echo === C: relallvisible before ===
select relname, relallvisible, relpages from pg_class where relname in ('contact_attempted','inquiry','person','correspondence_captured') order by 1;
\echo === D: simulate pre-vacuum visibility (relallvisible=0), default planner, jit off ===
SET jit = off; RESET enable_mergejoin;
UPDATE pg_class SET relallvisible = 0 WHERE relname in ('contact_attempted','inquiry','person','correspondence_captured','contact_method');
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF) EXECUTE builtin('40ca6bb3-5e13-487f-859a-bef64207a330', '79b710c9-f440-4299-aa35-6982e466ade0', '2026-09-06T12:00:00Z');
\echo === E: restore relallvisible via VACUUM (throwaway db) ===
VACUUM contact_attempted; VACUUM inquiry; VACUUM person; VACUUM correspondence_captured; VACUUM contact_method;
select relname, relallvisible from pg_class where relname in ('contact_attempted','inquiry','person') order by 1;
EXPLAIN (ANALYZE, BUFFERS, TIMING OFF) EXECUTE builtin('40ca6bb3-5e13-487f-859a-bef64207a330', '79b710c9-f440-4299-aa35-6982e466ade0', '2026-09-06T12:00:00Z');
