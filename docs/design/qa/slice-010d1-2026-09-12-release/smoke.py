...
def run():
    ...
    for base in (BASES[0], PUBLIC_API):
        ...
        for path in MIGRATION_READS:
            status, _, headers = req(base, path)
            require(status == 401, 'anonymous_migration_read_not_denied')
            if path.endswith('history-captures'):
                require(headers.get('cache-control') == 'no-store', 'anonymous_history_error_cacheable')
    ...
    for email, role, expected in (
        ...
        try:
            ...
            for path in MIGRATION_READS:
                ...
                if path.endswith('history-captures'):
                    require(headers.get('cache-control') == 'no-store', 'history_read_error_cacheable')
                if status == 200:
            ...
            for kind in ('metadata-imports', 'activity-imports', 'history-captures'):
                ...
                if kind in ('activity-imports', 'history-captures'):
                    require(headers.get('cache-control') == 'no-store', 'unknown_history_error_cacheable' if kind == 'history-captures' else 'unknown_activity_error_cacheable')
