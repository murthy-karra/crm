#!/usr/bin/env node
// Prepared only. Root must explicitly authorize execution after the 010d2 rollout.
...
const PRIVATE = '/private/tmp/crm-010d2-release-q9q7lzwf';
...
const proof = {
  kind: 'public read-only 010d2 deployment smoke', started_at: new Date().toISOString(),
...
async function instrument(context, actor) {
  ...
  context.on('response', response => {
    ...
    const expected403 = location.origin === API && location.path === '/api/migrations/fub/history-imports' && actor.label === 'carol' && status === 403 && observedPhase === 'carol_direct_api_denial';
...
async function emptyHistory(page, label) {
  const panel = page.getByTestId('history-import-panel');
  await panel.waitFor({ state: 'visible', timeout: 20000 });
  const parent = panel.getByLabel('People import for historical records', { exact: true });
  await parent.waitFor({ state: 'visible' });
  check((await parent.locator('option').count()) === 1 && (await parent.inputValue()) === '', `${label}_no_parent`);
  check(await panel.getByRole('button', { name: 'Prepare history import', exact: true }).count() === 0, `${label}_prepare_unavailable_without_parent`);
  check(await panel.getByLabel('Retained historical capture to import', { exact: true }).count() === 0, `${label}_capture_selector_unavailable_without_parent`);
  check(await panel.getByText('Preview retained events, calls and texts for a completed People import. No source connection is needed.', { exact: true }).isVisible(), `${label}_retained_scope_explained`);
  check(await panel.getByRole('alert').count() === 0, `${label}_no_panel_error`);
  check(await panel.getByLabel('Historical import attempt', { exact: true }).count() === 0, `${label}_no_import_attempt`);
  proof.checks.push({ label, no_completed_parent: true, prepare_unavailable_without_parent: true, no_import_attempt: true, capture_selector_unavailable_without_parent: true, panel_alerts: 0 });
  return panel;
}
...
async function layout(page, label, width, height) {
  ...
  const metrics = await page.evaluate(() => {
    const panel = document.querySelector('[data-testid="history-import-panel"]');
    return {
  ...
  await screenshot(page, panel, `history-import-empty-${label}.png`);
...
try {
  ...
  const [binary, web, deployed] = await Promise.all(['build-sha256.json', 'web-build-sha256.json', 'deployed.json'].map(metadata));
  ...
  for (const [path, digest] of Object.entries(web.value)) {
    ...
    check(sha(await readFile(join('/Users/karrad/projects/crm/web/dist', path))) === digest, 'local_web_manifest_mismatch');
  ...
  proof.release = {
    ...
    binary_provenance: 'Coordinator-owned deployment record; this browser does not inspect running processes.',
    expected_web_assets: expectedAssets.size,
    local_web_assets_match_manifest: true,
    local_index_sha256: expectedAssets.get('index.html'),
    metadata: [binary, web, deployed].map(x => ({ file: x.file, sha256: x.sha256 })),
  };
  proof.scope = 'Empty shared development only. Existing populated synthetic walkthrough is separate evidence. No source qualification or import execution is performed.';
  ...
  await runActor({ label: 'alice', email: 'alice@acme.test', role: 'admin' }, async page => {
    ...
    phase = 'alice_direct_history_import_read';
    const retainedList = await page.evaluate(async api => {
      const response = await fetch(`${api}/api/migrations/fub/history-imports`, { credentials: 'include', cache: 'no-store', redirect: 'error', signal: AbortSignal.timeout(15000) });
      const body = await response.json();
      return { status: response.status, no_store: response.headers.get('cache-control') === 'no-store', empty_exact: JSON.stringify(Object.keys(body).sort()) === JSON.stringify(['imports', 'next_cursor']) && Array.isArray(body.imports) && body.imports.length === 0 && body.next_cursor === null };
    }, API);
    check(retainedList.status === 200 && retainedList.no_store && retainedList.empty_exact, 'admin_history_imports_empty_no_store');
    proof.checks.push({ label: 'admin_history_imports_empty_no_store', ...retainedList });
  ...
  await runActor({ label: 'carol', email: 'carol@acme.test', role: 'member' }, async page => {
    ...
    check(await page.getByTestId('history-import-panel').count() === 0, 'member_no_history_panel');
    ...
    const denial = await page.evaluate(async api => {
      const response = await fetch(`${api}/api/migrations/fub/history-imports`, { credentials: 'include', cache: 'no-store', redirect: 'error', signal: AbortSignal.timeout(15000) });
      const body = await response.json();
      return { status: response.status, no_store: response.headers.get('cache-control') === 'no-store', closed_error: JSON.stringify(body) === JSON.stringify({ error: 'forbidden' }) };
    }, API);
    check(denial.status === 403 && denial.no_store && denial.closed_error, 'member_history_import_api_denial');
    proof.checks.push({ label: 'member_history_import_api_denial', ...denial });
  });
