...
def retirement_paths():
    ...
    for profile in ('debug', 'release'):
        ...
        for pattern in ('snapshot_qa*', 'import_qa*', 'metadata_import_qa*', 'activity_import_qa*', 'history_capture_qa*', 'history_import_qa*'):
...
def inventory():
    ...
    for line in subprocess.check_output(['ps', 'axo', 'pid=,comm=']).decode().splitlines():
        ...
        if name not in ('crm-api', 'crm-admin', 'migrate'):
            assert not re.match(r'^(crm_api-|crm_admin-|migrate-|snapshot_qa|import_qa|metadata_import_qa|activity_import_qa|history_capture_qa|history_import_qa)', name), 'Unexpected alternate CRM runtime'
            continue
    ...
    assert not any(token in cron.stdout.lower() for token in [b'/projects/crm', b'crm-api', b'crm-admin', b'activity_import_qa', b'history_capture_qa', b'history_import_qa']), 'Re-inventory scheduled CRM cron launch paths'
...
def preflight(purpose):
    ...
    if purpose == 'confirm':
        ...
        assert report['history_timeline_confirmation_ready']
        assert report['history_timeline_confirmation_reasons'] == []
        assert report['history_timeline_schema_present']
        assert report['history_timeline_unsupported_count'] == '0'
...
def deploy():
    ...
    assert values()['CRM_MIGRATION_RELEASE_REPORT'] == json.loads((RELEASE / 'release-before.json').read_text())['report_path'], 'Unexpected current report path'
    lines[indices[0]] = 'CRM_MIGRATION_RELEASE_REPORT=' + str(RELEASE / 'release-report.json')
    lines = [line.replace('# 010d1 operator-owned compatibility evidence;', '# 010d2 operator-owned compatibility evidence;') for line in lines]
    config.write_text('\n'.join(lines) + '\n')
    config.chmod(0o600)
