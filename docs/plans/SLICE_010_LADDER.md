...
implementation and synthetic verification. 010b passed both bounded reviews and
...
| 010f+ | One rung per remaining entity as destinations ship: tags (destination since 011e) / custom fields (destination since Slice 019a, D-058: `custom_field.source`/`external_key` and value `origin`/`correlation_id` are the import hooks), FUB email records → correspondence metadata (bodies O-012), tasks/appointments, deals (thesis §11 defers Deal model), notes bodies (O-012). Each from a fresh delta fetch. | S each |
