...
| `GET /api/people` | — *`?filter=<percent-encoded JSON>` added by SLICE_011a §5a (declared additive, AGENTS.md §11): absent = this row, byte-identical; present = a validated `FilterDefinition`, narrowing rows only — same shape, order, cap.* | 200 `{"people":[{id, first_name, last_name, display_name, stage:{id,name}, assigned_user:{id,display_name}\|null, primary_email, primary_phone, inquiry_count, last_inquiry_at, created_at}], "truncated": bool}` ordered `created_at DESC, id`; cap 500 | *SLICE_011a §5a adds 400 `malformed_request` (undecodable/structurally invalid filter) and 422 `invalid_stage`/`invalid_assignee` (org-scoped filter ids), present-`filter`-only.* |
...
| `GET /api/inquiry-sources` *(added by SLICE_011a §5b, declared additive, AGENTS.md §11)* | — | 200 `{"sources": ["email", "website", …], "truncated": bool}` — org-scoped `DISTINCT source`, ascending; cap 500. Any authenticated member; org from session only, like `GET /api/stages`. Feeds the People FilterBar's Source picker. | |
