...
import { activityActive, cancelAdmittedActivityImport, confirmAdmittedActivityImport, fetchActivityField, fetchAdmittedActivityImport, fetchAdmittedActivityImports, fetchActivityMappings, fetchActivityObservations, fetchActivityRecords, fetchActivityResults, fetchActivityTargets, proposeAdmittedActivityImport, replanAdmittedActivityImport, retryAdmittedActivityImport, useActivityAccess, type AdmittedActivityImport, type ActivityReplan } from './admittedActivityImports'
...
describe('activity import transport', () => {
  it('preserves decimal acknowledgments, explicit mapping variants and omitted versus cleared timezone', async () => {
    ...
    await proposeAdmittedActivityImport({ request_id: 'proposal', admission_id: 'admission', report_id: 'report' }, signal)
    await replanAdmittedActivityImport('child/1', body, signal)
    await replanAdmittedActivityImport('child/1', { ...body, source_timezone: null }, signal)
    await replanAdmittedActivityImport('child/1', { ...body, source_timezone: 'America/Los_Angeles' }, signal)
    await confirmAdmittedActivityImport('child/1', confirm, signal)
    await confirmAdmittedActivityImport('child/1', confirm, signal)
    await retryAdmittedActivityImport('child/1', { request_id: 'retry', expected_revision: revision }, signal)
    await cancelAdmittedActivityImport('child/1', { request_id: 'cancel', expected_revision: revision }, signal)
  ...
  it('constructs all scoped bounded routes without following source URLs', async () => {
    const signal = new AbortController().signal
    await fetchAdmittedActivityImports('parent/1', 'c +/=', signal); await fetchAdmittedActivityImport('i/1', signal)
  ...
  it('propagates uncertain errors without silently retrying', async () => {
    ...
    await expect(retryAdmittedActivityImport('i', { request_id: 'exact', expected_revision: revision })).rejects.toBe(error)
    expect(api).toHaveBeenCalledTimes(1)
  })
  it('uses its own cache namespace and polls only active work', () => {
    useActivityAccess(); expect(useImportAccess).toHaveBeenCalledWith('admitted-activity-imports')
    for (const state of ['preparing', 'queued', 'running', 'ready', 'paused', 'completed', 'cancelled'] as const) expect(activityActive({ state } as AdmittedActivityImport)).toBe(['preparing', 'queued', 'running'].includes(state))
  })
})
