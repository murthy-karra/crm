...
export interface ActivityCounts { notes: ActivityFamilyCounts; tasks: ActivityFamilyCounts; held_count: string; source_only_count: string; invalid_occurrences: string; excluded_count?: string; unavailable_bodies: string }
...
export interface AdmittedActivityImport {
...
export interface AdmittedActivityAction { request_id: string; expected_revision: string }
export interface ActivityEnvelope { import: AdmittedActivityImport }
export interface ActivityPage<T> { items: T[]; next_cursor: string | null }
export interface ActivityList { imports: AdmittedActivityImport[]; next_cursor: string | null }
...
export interface ActivityRecord { id: string; kind: ActivityKind; source_id: string | null; person_id: string | null; target_id: string | null; disposition: 'eligible' | 'already_present' | 'held' | 'excluded'; source_summary: MetadataSummary; preview: ActivityPreview; field_url: string; observations_url: string }
...
export const fetchAdmittedActivityImports = (admission?: string, cursor?: string, signal?: AbortSignal) => apiFetch<ActivityList>(root + query({ admission_id: admission, cursor, limit: 20 }), { signal })
export const fetchAdmittedActivityImport = (id: string, signal?: AbortSignal) => apiFetch<AdmittedActivityImport>(path(id), { signal })
export const proposeAdmittedActivityImport = (body: { request_id: string; admission_id: string; report_id: string }, signal?: AbortSignal) => post(root, body, signal)
export const replanAdmittedActivityImport = (id: string, body: ActivityReplan, signal?: AbortSignal) => post(`${path(id)}/plans`, body, signal)
...
export const useActivityAccess = () => useImportAccess('admitted-activity-imports')
export const activityActive = (r?: AdmittedActivityImport) => !!r && ['preparing', 'queued', 'running'].includes(r.state)
