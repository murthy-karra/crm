...
import type { ReviewFullNote, ReviewNote, ReviewTask } from '../api/activityReview'
import { historyKinds, type HistoryReviewCore } from '../api/historyReview'
...
function core(revision = '0', person = 'person'): HistoryReviewCore {
  return {
    ...
    contact_methods: [{ id: 'email', kind: 'email', value: 'plain@example.invalid' }], inquiries: { count: '0', url: '/unused' }, tags: [], custom_fields: [], history: { read_revision: '0', known_count: '0', unknown_count: '0', counts: Object.fromEntries(historyKinds.map(kind => [kind, '0'])) as HistoryReviewCore['history']['counts'], timeline_url: '/unused' },
...
describe('bounded Person activity review', () => {
  ...
  it('clears every page and full body on stale-cursor 409 before refetching core and page one', async () => {
    const fresh = deferred<HistoryReviewCore>(); let coreReads = 0
    const { wrapper } = await setup(url => {
      if (url.endsWith('/migration-review/v2')) return ++coreReads === 1 ? core('1') : fresh.promise
  ...
  it.each(['person', 'actor', 'org', 'role', 'workspace', 'session'] as const)('rejects a late full body after %s changes', async transition => {
    ...
    const { wrapper, client } = await setup(url => url.endsWith('/migration-review/v2') ? core('1', url.includes('/another/') ? 'another' : 'person') : url.endsWith('/notes/note') ? delayed.promise : page(url.includes('/notes?') ? [note()] : [], '1'))
    await click('Read full note')
  ...
  it('refreshes core on focus and reconnect and restarts pages when its revision changes', async () => {
    let revision = '1'
    const { wrapper } = await setup(url => url.endsWith('/migration-review/v2') ? core(revision) : page(url.includes('/notes?') ? [note(`revision-${revision}`)] : [], revision))
    ...
    expect(api.mock.calls.filter(([url]) => url.endsWith('/migration-review/v2'))).toHaveLength(3)
  ...
  it('rejects an earlier response after closing and reopening the same full note', async () => {
    ...
    await setup(url => url.endsWith('/migration-review/v2') ? core() : url.endsWith('/notes/note') ? ++noteReads === 1 ? old.promise : full('note', 'CURRENT FULL BODY') : page(url.includes('/notes?') ? [note()] : []))
