...
const reading = useQuery({ queryKey: key, queryFn: ({ signal }) => access.read(key.value, () => key.value, () => fetchActivityField(props.request, cursors.value.at(-1) || undefined, signal)), enabled: access.enabled, retry: false, gcTime: 0 })
  ...
  >
    ...
    <template v-if="segment">
      <p class="mt-2 text-small text-text-muted">
        Bytes {{ segment.offset }}–{{ end }} of {{ segment.total_utf8_bytes }}. One segment is shown at a time.
      </p>
      <pre
        tabindex="0"
        aria-label="Activity source field text"
