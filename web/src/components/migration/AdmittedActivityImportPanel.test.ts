...
import type { ActivityCounts, AdmittedActivityImport } from '../../api/admittedActivityImports'
import { resetWorkspace } from '../../workspaceLifecycle'
import AdmittedActivityImportPanel from './AdmittedActivityImportPanel.vue'
vi.mock('../../api/client', async original => ({ ...await original<typeof import('../../api/client')>(), apiFetch: vi.fn() }))
const api = vi.mocked(apiFetch)
const ROOT = '/migrations/fub/admitted-activity-imports'
...
function run(state: AdmittedActivityImport['state'] = 'ready'): AdmittedActivityImport {
  const confirmed = ['queued', 'running', 'completed'].includes(state)
  return { id: 'child', parent_import_id: 'parent', parent_plan_id: 'parent-plan', admission_id: 'admission', admission_plan_id: 'admission-plan', source_report_id: 'report', source_output_revision: 'output', snapshot_id: 'snapshot', source_account_id: '900719925474099312345', capture_sequence: '9', workspace_revision: '2', revision: '9007199254740995', activity_revision: '1', engine_version: 'fub-admitted-activity-v1', state, phase: confirmed ? 'records' : 'preparation', pause_reason: state === 'paused' ? 'storage_limit' : null, created_at: '2026-09-11T12:00:00Z', updated_at: '2026-09-11T12:00:00Z', confirmed_at: null, confirmed_plan_id: confirmed ? 'plan' : null, completed_at: null, retained_bytes: '1024', reserved_bytes: '65536', native_row_bytes: '0', cancellation_reserved_bytes: '65536', release_ready: true, counts: counts(), latest_plan: { id: 'plan', revision: '9007199254740993', state: state === 'preparing' ? 'building' : 'ready', phase: 'ready', expires_at: '2099-01-01T12:00:00Z', counts: counts(), source_timezone: null, source_engine: 'source', html_profile: 'html', time_profile: 'time', tzdb_version: '2025b', confirmation_digest: 'digest', max_added_byte_bound: '10000' }, actions: { replan: ['ready', 'preparing'].includes(state), confirm: state === 'ready', retry: state === 'paused', cancel: !['completed', 'cancelled'].includes(state), remainder: false }, policy: { run_byte_limit: '268435456', org_byte_limit: '1073741824', run_ceiling_bytes: '536870912', org_ceiling_bytes: '1073741824', run_retained_bytes: '1024', run_reserved_bytes: '65536', org_retained_bytes: '1024', org_reserved_bytes: '65536', unit_byte_limit: '67108864', policy_revision: 'policy' }, coverage: { remaining_data: ['note_replies', 'recurrence', 'activation'], native_review_only: true }, remainder: { available: false } }
}
...
async function setup(options: { run?: AdmittedActivityImport | null; role?: string; missingStream?: string; snapshotState?: string; parentState?: string; real?: boolean; handle?: (url: string, init?: RequestInit) => unknown } = {}) {
  ...
  const admission = { id: 'admission', parent_import_id: 'parent', state: 'completed', created_at: '2026-09-11T11:00:00Z', progress: { settled_items: '2' } }
  const report = { id: 'report', parent_import_id: 'parent', parent_plan_id: 'parent-plan', source_account_id: '900719925474099312345', state: 'completed', output_revision: 'output', inputs: { newer: { snapshot_id: 'snapshot', started_at: '2026-09-11T11:00:00Z', streams: ['people', 'users', 'notes', 'note_detail', 'tasks_open', 'tasks_completed'].map(stream => ({ stream, state: stream === options.missingStream ? 'paused' : 'completed' })) } } }
  api.mockImplementation(async (url, init) => {
    ...
    if (url === '/migrations/fub/people-admissions/admission') return admission as never
    if (url === '/migrations/fub/core-change-reports?parent_import_id=parent&limit=20') return { reports: [report], next_cursor: null } as never
    if (url === '/migrations/fub/core-change-reports/report') return report as never
...
describe('activity import workflow', () => {
  ...
  it.each(['people', 'users', 'notes', 'note_detail', 'tasks_open', 'tasks_completed'])('requires exhausted %s alongside the other five streams', async missingStream => { await setup({ run: null, missingStream }); expect(button('Prepare admitted notes and tasks plan').disabled).toBe(true) })
  it.each([{ parentState: 'paused' }, { parentState: 'cancelled' }])('rejects an ineligible People import %o', async options => { await setup({ run: null, ...options }); expect(button('Prepare admitted notes and tasks plan').disabled).toBe(true); expect(writes()).toHaveLength(0) })
  ...
  it('discards late receipts and private state after the actor changes', async () => {
    const pending = deferred<{ import: AdmittedActivityImport }>()
    ...
    expect(client.getQueryCache().findAll({ queryKey: ['org', 'org', 'admitted-activity-imports'] }).some(q => q.state.data !== undefined)).toBe(false)
  })
