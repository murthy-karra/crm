...
import type { MetadataCounts } from '../../api/metadataImports'
import type { AdmittedMetadataImport as MetadataImport } from '../../api/admittedMetadataImports'
import { resetWorkspace } from '../../workspaceLifecycle'
import AdmittedMetadataImportPanel from './AdmittedMetadataImportPanel.vue'
...
const boundary = { snapshot_id: 'snapshot', capture_sequence: '8', profile_version: 'profile', schema_version: 'schema', started_at: '2026-09-11T10:00:00Z', completed_at: '2026-09-11T11:00:00Z', streams: [{ stream: 'people', state: 'completed', reported_total: '5', returned_items: '5', content_gaps: '0', accepted_captures: '1' }, { stream: 'custom_fields', state: 'completed', reported_total: '3', returned_items: '3', content_gaps: '0', accepted_captures: '1' }] }
function run(state: MetadataImport['state'] = 'proposed'): MetadataImport {
  const confirmed = ['queued', 'running', 'completed'].includes(state)
  return { id: 'child', parent_import_id: 'parent', parent_plan_id: 'parent-plan', snapshot_id: 'snapshot', source_account_id: '900719925474099312345', capture_sequence: '8', workspace_revision: '2', engine_version: 'fub-admitted-metadata-v1', admission_id: 'admission', admission_plan_id: 'admission-plan', source_report_id: 'report', source_output_revision: '3', source_capture_interval: boundary, shared_claims_ready: true, cohort_counts: { settled_people: '5', eligible_people: '4', excluded_people: '1', settled_metadata_people: '0', remaining_people: '4' }, progress: { phase: 'fields', fields_processed: '3', people_processed: '5' }, remainder: { available: false, predecessor_import_id: null, successor_import_id: null, remaining_catalog: '0', remaining_people: '0', excluded_settled_catalog: '0', excluded_settled_people: '0', excluded_held_people: '0' }, state, phase: confirmed ? 'people' : 'preparation', pause_reason: state === 'paused' ? 'storage_limit' : null, created_at: '2026-09-11T12:00:00Z', updated_at: '2026-09-11T12:00:00Z', confirmed_at: null, completed_at: null, confirmed_plan_id: confirmed ? 'plan' : null, retained_bytes: '1024', reserved_bytes: '65536', cancellation_reserved_bytes: '65536', release_ready: true, counts: counts(), latest_plan: { id: 'plan', revision: '9007199254740993', state: 'ready', phase: 'ready', pause_reason: null, expires_at: '2099-01-01T12:00:00Z', confirmation_digest: 'exact-digest', counts: counts(), max_added_byte_bound: '3145728' }, actions: { remainder: false, confirm: state === 'proposed', replan: state === 'proposed', retry: state === 'paused', cancel: !['completed', 'cancelled'].includes(state) }, policy: { run_byte_limit: '2147483648', org_byte_limit: '4294967296', run_ceiling_bytes: '2147483648', org_ceiling_bytes: '4294967296', run_retained_bytes: '1024', run_reserved_bytes: '65536', org_retained_bytes: '1024', org_reserved_bytes: '65536', unit_byte_limit: '67108864', policy_revision: 'policy' }, coverage: { embedded_tags_only: true, custom_fields_complete: true, metadata_excluded_people: '1', remaining_data: ['notes', 'tasks', 'emails'], source_gaps: ['notes_unavailable'] } }
}
...
async function setup(options: { run?: MetadataImport | null; role?: string; completeFields?: boolean; parentState?: string; realChildren?: boolean; handle?: (url: string, init?: RequestInit) => unknown } = {}) {
  ...
  api.mockImplementation(async (url, init) => {
    ...
    const cohort = { id: 'admission', parent_import_id: 'parent', report_id: 'report', state: 'completed', progress: { settled_items: '5' }, source_boundary: { newer: { completed_at: boundary.completed_at } } }
    const retained = { id: 'report', state: 'completed', parent_import_id: 'parent', inputs: { newer: { ...boundary, streams: boundary.streams.map(stream => ({ ...stream, state: stream.stream === 'custom_fields' && options.completeFields === false ? 'paused' : 'completed' })) } } }
    if (url === '/migrations/fub/people-admissions?parent_import_id=parent&limit=20') return { items: [cohort], next_cursor: null } as never
    if (url === '/migrations/fub/people-admissions/admission') return cohort as never
    if (url === '/migrations/fub/core-change-reports?parent_import_id=parent&limit=20') return { reports: [retained], next_cursor: null } as never
    if (url === '/migrations/fub/core-change-reports/report') return retained as never
...
describe('AdmittedMetadataImportPanel', () => {
  it('automatically refreshes same-plan mappings and planned People when preparation becomes ready', async () => {
    ...
    expect(api.mock.calls.some(([url]) => url.startsWith('/migrations/fub/metadata-imports'))).toBe(false)
  ...
  it('only prepares from the completed parent and complete field-definition stream', async () => {
    ...
    expect(writes()).toHaveLength(0); expect(button('Prepare admitted metadata plan').disabled).toBe(false)
    button('Prepare admitted metadata plan').click(); await flushPromises()
    expect(JSON.parse(String(writes()[0]![1]?.body))).toEqual({ request_id: 'request-uuid', admission_id: 'admission', source_report_id: 'report' })
  ...
  it.each([{ completeFields: false }, { parentState: 'paused' }])('blocks ineligible retained evidence %o', async options => {
    await setup({ run: null, ...options }); expect(button('Prepare admitted metadata plan').disabled).toBe(true); expect(writes()).toHaveLength(0)
  })
  ...
  it('clears frozen requests and private summaries when the actor loses admin access', async () => {
    ...
    for (const query of client.getQueryCache().findAll({ queryKey: ['org', 'org', 'admitted-metadata-imports'] })) expect(query.state.data).toBeUndefined()
  })
  it('reprepares an unconfirmed cancellation without changing its retained source implicitly', async () => {
    const { wrapper } = await setup({ run: run('cancelled') })
    expect(wrapper.text()).toContain('This attempt is cancelled')
    expect(button('Prepare admitted metadata plan').disabled).toBe(false)
    expect(wrapper.text()).not.toContain('Resume metadata import'); expect(writes()).toHaveLength(0)
  })
  it('requires explicit approval for one exact confirmed remainder and replays its uncertain receipt', async () => {
    let attempts = 0
    const cancelled = run('cancelled'); cancelled.confirmed_plan_id = 'plan'; cancelled.actions.remainder = true
    cancelled.remainder = { ...cancelled.remainder, available: true, remaining_catalog: '2', remaining_people: '3', excluded_settled_catalog: '4', excluded_settled_people: '1', excluded_held_people: '1' }
    const { wrapper } = await setup({ run: cancelled, handle: url => {
      if (url.endsWith('/remainder')) return ++attempts === 1 ? Promise.reject(new ApiError(0, 'network_error')) : { import: { ...run('queued'), id: 'successor' } }
      if (url === `${ROOT}/successor`) return { ...run('running'), id: 'successor' }
    } })
    expect(button('Prepare admitted metadata plan').disabled).toBe(true)
    button('Review exact remainder').click(); await flushPromises(); expect(writes()).toHaveLength(0)
    expect(document.body.textContent).toContain('2 remaining catalog units and 3 People')
    button('Continue exact remainder').click(); await flushPromises()
    button('Retry the same metadata request').click(); await flushPromises()
    expect(writes('/remainder')).toHaveLength(2)
    expect(writes('/remainder')[0]![1]?.body).toBe(writes('/remainder')[1]![1]?.body)
    expect(wrapper.text()).toContain('successor'); expect(writes('/confirm')).toHaveLength(0)
  })
  it('makes resume and terminal cancellation explicit and never starts work on allowance refresh', async () => {
    ...
    button('Cancel remaining work').click(); await flushPromises(); expect(writes('/cancel')).toHaveLength(1)
  })
  ...
  it('requires a fresh counted plan after selecting a different retained source', async () => {
    const later = { id: 'later', parent_import_id: 'parent', state: 'completed', inputs: { newer: { ...boundary, snapshot_id: 'later-snapshot', capture_sequence: '10' } } }
    const { wrapper, state } = await setup({ handle: (url, init) => {
      if (url === '/migrations/fub/core-change-reports?parent_import_id=parent&limit=20') return { reports: [{ ...later, id: 'report' }, later], next_cursor: null }
      if (url === '/migrations/fub/core-change-reports/later') return later
      if (url.endsWith('/plans') && init?.method === 'POST') {
        state.run = { ...run(), source_report_id: 'later', latest_plan: { ...run().latest_plan!, revision: '9007199254740994', confirmation_digest: 'later-digest' } }
        return { import: state.run }
      }
    } })
    await acknowledge(wrapper)
    const source = wrapper.findAll('select').find(select => select.find('option[value="later"]').exists())!
    await source.setValue('later'); await flushPromises()
    expect(document.body.querySelector('[role=dialog]')).toBeNull()
    expect(button('Review metadata confirmation').disabled).toBe(true)
    button('Prepare plan from selected source').click(); await flushPromises()
    expect(JSON.parse(String(writes('/plans')[0]![1]?.body))).toEqual({ request_id: 'request-uuid', expected_plan_revision: '9007199254740993', mappings: [], source_report_id: 'later' })
    expect(button('Review metadata confirmation').disabled).toBe(true)
    expect(writes('/confirm')).toHaveLength(0)
  })
  it('keeps expired plans readable while preventing a new confirmation', async () => {
    const expired = run(); expired.latest_plan!.expires_at = '2000-01-01T00:00:00Z'
    const { wrapper } = await setup({ run: expired })
    for (const checkbox of wrapper.findAll('input[type=checkbox]')) await checkbox.setValue(true)
    expect(wrapper.text()).toContain('This ready plan has expired')
    expect(button('Review metadata confirmation').disabled).toBe(true)
    expect(writes()).toHaveLength(0)
  })
