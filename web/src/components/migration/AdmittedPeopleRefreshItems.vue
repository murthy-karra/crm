...
import AdmittedPeopleRefreshItemDetail from './AdmittedPeopleRefreshItemDetail.vue'
import { buttonClasses, INPUT_CLASSES } from '../../lib/controls'
import { describeApiError } from '../../lib/errors'
import { importAccessError } from '../../api/imports'
import { fetchAdmittedPeopleRefreshItems, fetchAdmittedPeopleRefreshResults, refreshDispositions, refreshLabel, useAdmittedPeopleRefreshAccess, type AdmittedPeopleRefresh, type RefreshDisposition, type RefreshPlan } from '../../api/admittedPeopleRefreshes'
import { snapshotTime } from './format'
const props = defineProps<{ refresh: AdmittedPeopleRefresh; plan: RefreshPlan }>()
...
const firstResults = ref<Awaited<ReturnType<typeof fetchAdmittedPeopleRefreshResults>> | null>(null)
...
const items = useQuery({ queryKey: itemsKey, enabled: access.enabled, retry: false, gcTime: 0, staleTime: Infinity, refetchOnWindowFocus: false, refetchOnReconnect: false, queryFn: ({ signal }) => access.read(itemsKey.value, () => itemsKey.value, async () => {
  const value = await fetchAdmittedPeopleRefreshItems(props.refresh.id, filter.value || undefined, pages.value.at(-1) || undefined, signal)
...
const results = useQuery({ queryKey: resultsKey, enabled: computed(() => access.enabled.value && ['running', 'paused', 'completed', 'cancelled'].includes(props.refresh.state)), retry: false, gcTime: 0, staleTime: Infinity, refetchOnWindowFocus: false, refetchOnReconnect: false, queryFn: ({ signal }) => access.read(resultsKey.value, () => resultsKey.value, async () => {
  ...
  const value = await fetchAdmittedPeopleRefreshResults(props.refresh.id, resultPages.value.at(-1) || undefined, signal)
  ...
  >
    ...
    >
      ...
      >
        ...
        <AdmittedPeopleRefreshItemDetail
