...
import { fetchPeopleRefreshField, refreshLabel, usePeopleRefreshAccess, type RefreshPlan } from '../../api/peopleRefreshes'
...
const result = useQuery({ queryKey: key, enabled: access.enabled, retry: false, gcTime: 0, staleTime: Infinity, refetchOnWindowFocus: false, refetchOnReconnect: false, queryFn: ({ signal }) => access.read(key.value, () => key.value, async () => {
  const value = await fetchPeopleRefreshField(props.refreshId, props.itemId, props.side, props.field, pages.value.at(-1) || undefined, signal)
