...
import PeopleRefreshFieldFragment from './PeopleRefreshFieldFragment.vue'
import { useQuery } from '@tanstack/vue-query'
import { buttonClasses } from '../../lib/controls'
import { describeApiError } from '../../lib/errors'
import { importAccessError } from '../../api/imports'
import { fetchPeopleRefreshContacts, fetchPeopleRefreshItem, refreshLabel, usePeopleRefreshAccess, type RefreshPlan, type RefreshProjection } from '../../api/peopleRefreshes'
const props = defineProps<{ refreshId: string; itemId: string; plan: RefreshPlan }>()
const emit = defineEmits<{ accessDenied: [] }>()
const access = usePeopleRefreshAccess(); const pages = ref([''])
...
const detail = useQuery({ queryKey: detailKey, enabled: access.enabled, retry: false, gcTime: 0, staleTime: Infinity, refetchOnWindowFocus: false, refetchOnReconnect: false, queryFn: ({ signal }) => access.read(detailKey.value, () => detailKey.value, async () => verified(await fetchPeopleRefreshItem(props.refreshId, props.itemId, signal))) })
const contacts = useQuery({ queryKey: contactKey, enabled: computed(() => access.enabled.value && !!detail.data.value), retry: false, gcTime: 0, staleTime: Infinity, refetchOnWindowFocus: false, refetchOnReconnect: false, queryFn: ({ signal }) => access.read(contactKey.value, () => contactKey.value, async () => verified(await fetchPeopleRefreshContacts(props.refreshId, props.itemId, pages.value.at(-1) || undefined, signal))) })
