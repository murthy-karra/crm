...
import type { PeopleRefresh, RefreshItemDetail } from '../../api/peopleRefreshes'
import PeopleRefreshPanel from './PeopleRefreshPanel.vue'
vi.mock('../../api/client', async original => ({ ...await original<typeof import('../../api/client')>(), apiFetch: vi.fn() }))
const api = vi.mocked(apiFetch); const ROOT = '/migrations/fub/people-refreshes'; const REPORTS = '/migrations/fub/core-change-reports'
...
async function setup(options: { state?: PeopleRefresh['state']; role?: string; handle?: (url: string, init?: RequestInit) => unknown } = {}) {
  ...
  api.mockImplementation(async (url, init) => {
    ...
    if (url === `${ROOT}?parent_import_id=parent&limit=20`) return { refreshes: [state.refresh], next_cursor: null } as never
  ...
  const wrapper = mount(PeopleRefreshPanel, { props: { refreshWorkspace }, global: { plugins: [[VueQueryPlugin, { queryClient: client }], [PrimeVue, { unstyled: true }]], stubs: { RouterLink: RouterLinkStub } }, attachTo: document.body })
...
describe('People refresh workflow', () => {
  it('prepares only a selected sealed report and obtains current state after the receipt', async () => {
    ...
    expect(JSON.parse(String(writes()[0]![1]?.body))).toEqual({ report_id: 'report', request_id: expect.stringMatching(/^request-\d+$/) })
  ...
  it('confirms the frozen repair counts including unchanged unassigned approvals', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); await acknowledge()
  ...
  it('replays an uncertain confirmation with identical input and request ID', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); await acknowledge(); button('Review confirmation').click(); await flushPromises(); button('Confirm exact People plan').click(); await flushPromises()
  ...
  it('requires a new preview when expired and never queues a no-change plan', async () => {
    ...
    state.refresh.plan!.expires_at = '2020-01-01T00:00:00Z'; await choose('Saved People refresh', 'refresh')
    ...
    button('Reload People refreshes').click(); await flushPromises(); await acknowledge(); expect(button('Review confirmation').disabled).toBe(true)
  ...
  it('uses expected plan/lifecycle revisions for re-preview, retry and explicitly confirmed cancellation', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); button('Prepare a new preview').click(); await flushPromises()
    expect(JSON.parse(String(writes()[0]![1]?.body))).toEqual({ request_id: expect.stringMatching(/^request-\d+$/), expected_plan_revision: 2 })
    state.refresh = refresh('paused'); button('Reload People refreshes').click(); await flushPromises(); button('Resume People refresh').click(); await flushPromises()
  ...
  it('drops late plaintext and receipts after an actor/Organization transition', async () => {
    ...
    expect(client.getQueryCache().findAll({ queryKey: ['org', 'org', 'people-refreshes'] })).toHaveLength(0)
  })
  it('fences a late detail after a disposition change and preserves opaque page filters', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); button('Inspect Person preview').click(); await flushPromises(); await choose('People preview disposition', 'held_local_change')
  ...
  it('rejects a wrong plan revision instead of displaying its values', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); expect(wrapper.text()).not.toContain('wrong-plan-private'); expect(wrapper.text()).toContain('preview could not be verified')
  })
  it('removes already displayed values when a current read denies access', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); expect(wrapper.text()).toContain('9007199254740993123'); revoked = true
    button('Reload People refreshes').click(); await flushPromises(); expect(wrapper.text()).not.toContain('9007199254740993123'); expect(wrapper.text()).toContain('currently verified Organization administrator')
  })
})

describe('People refresh traversal and late actions', () => {
  ...
  it('rejects a name fragment for another side and drops a late fragment after authority changes', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); button('Inspect Person preview').click(); await flushPromises(); button('Read complete name in pages').click(); await flushPromises()
  ...
  it('keeps the first results page fixed until an explicit new traversal', async () => {
    ...
    await choose('Saved People refresh', 'refresh'); button('More settled results').click(); await flushPromises(); button('Previous settled results').click(); await flushPromises()
