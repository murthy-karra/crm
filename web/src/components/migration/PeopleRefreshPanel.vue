...
import { cancelPeopleRefresh, confirmPeopleRefresh, fetchPeopleRefresh, fetchPeopleRefreshes, preparePeopleRefresh, refreshActive, refreshInteger, refreshLabel, repreviewPeopleRefresh, retryPeopleRefresh, usePeopleRefreshAccess, type RefreshConfirm } from '../../api/peopleRefreshes'
...
const parentId = ref(''); const reportId = ref(''); const refreshId = ref('')
...
type Intent = { identity: string; parent: string } & (
  { kind: 'prepare'; report: string; request: string }
...
const firstRefreshPage = ref<Awaited<ReturnType<typeof fetchPeopleRefreshes>> | null>(null)
const listing = useQuery({ queryKey: listKey, enabled: computed(() => access.enabled.value && !!parentId.value), retry: false, gcTime: 0, staleTime: Infinity, refetchOnWindowFocus: false, refetchOnReconnect: false, queryFn: ({ signal }) => access.read(listKey.value, () => listKey.value, async () => {
...
async function submit(value: Intent) {
  ...
  try {
    const result = value.kind === 'prepare' ? await preparePeopleRefresh(value.report, value.request)
      : value.kind === 'confirm' ? await confirmPeopleRefresh(value.id, value.body)
        : value.kind === 'plans' ? await repreviewPeopleRefresh(value.id, value.request, value.revision)
          : value.kind === 'retry' ? await retryPeopleRefresh(value.id, value.request, value.revision)
            : await cancelPeopleRefresh(value.id, value.request, value.revision)
    ...
    actionError.value = uncertain.value ? 'The outcome is uncertain. Retry the same request to recover its saved receipt.'
      : error instanceof ApiError && error.status === 409 ? 'The plan, destination, authority, storage allowance or release changed. Reload and review before continuing.'
        : describeApiError(error, 'Could not complete the People refresh request.')
    if (sameRequest()) reload()
  } finally { if (sameIdentity()) pending.value = false }
}
function prepare() { if (canPrepare.value) void submit({ kind: 'prepare', identity: access.identity.value, parent: parentId.value, report: reportId.value, request: crypto.randomUUID() }) }
...
<template>
  ...
  >
    ...
    >
      People refresh evidence is available only to a currently verified Organization administrator.
    </p>
    <ConfirmDialog
