...
import PersonAdmittedMetadataProvenance from './PersonAdmittedMetadataProvenance.vue'
...
async function setup(handle: (url: string) => unknown, role = 'admin') {
  ...
  const wrapper = mount(PersonAdmittedMetadataProvenance, { props: { personId: 'person' }, global: { plugins: [[VueQueryPlugin, { queryClient: client }]] } })
...
describe('PersonAdmittedMetadataProvenance', () => {
  it('shows source and applied values separately and reads the exact committed result field', async () => {
    ...
    expect(api.mock.calls.some(([url]) => url === '/people/person/admitted-metadata-import-provenance/result/fields/source.h?limit=65536')).toBe(true)
