...
import type { MetadataResult } from '../../api/metadataImports'
import { fetchAdmittedMetadataProvenance, useAdmittedMetadataAccess, admittedMetadataReaders } from '../../api/admittedMetadataImports'
...
const reading = useQuery({ queryKey: key, enabled: computed(() => access.enabled.value && !!props.personId), retry: false, gcTime: 0, queryFn: ({ signal }) => access.read(key.value, () => key.value, () => fetchAdmittedMetadataProvenance(props.personId, cursors.value.at(-1) || undefined, signal)) })
...
<template>
  <section
    ...
    class="min-w-0"
    aria-label="Later-admitted tags and custom fields"
  >
    <h2 class="text-section font-semibold">
      Later-admitted tags and custom fields
    </h2>
    <p class="mt-1 text-small text-text-muted">
      Retained admission source evidence and committed metadata results. This workspace remains for administrator review.
    </p>
    <p
